...
| `AcpPermission` | `src/providers/acpPermission.ts` | The ACP authorization policy — **delegated execution is never delegated authorization**. Maps ACP `ToolKind` onto AtlasMind's own `ToolRiskCategory` so a bypass means the same thing for a delegated agent as for a subtask (`ToolKind::Other` is `#[serde(other)]`, so it is the *unidentifiable* bucket ⇒ highest risk, not lowest). **Never selects `allow_always`** — that grant would live in the agent's persistent state, unrevokable from AtlasMind; declines outright if it is the only way to approve. Unreadable request ⇒ refused, never a fabricated `optionId`. `selectAcpMcpServers` shares only allowlisted stdio servers and never one holding SecretStorage credentials. Pure + unit-tested; the dialog and settings gate live at the call site |
