...
8. **Refresh the README's published baseline** to the version just tagged, and commit it.

   This step is not optional housekeeping — **the repository's own test suite is red until it is done.**
   `docsIntegrity` asserts that the "last Marketplace publication, **vX**" line in `README.md` names the
...
| `TestingFrameworkDetection` / `TestingFrameworkSyntax` | `src/core/testingFrameworkDetection.ts`, `src/core/testingFrameworkSyntax.ts` | Which runner the Scaffold button uses, and what dialect it writes in. The choice was `has('vitest') ? 'vitest' : has('jest') ? 'jest' : undefined` with `?? 'vitest'` downstream — two runners out of six, so a Mocha project got Vitest files it could not run and a project already on Vitest could be handed `npm install -D jest`, which is how a repository acquires two runners and a suite only one can execute. Four rules. **What the project already uses always wins** — detection outranks preference entirely, and the recommendation ladder (Vite→Vitest, React-without-Vite→Jest, meta-framework→Vitest) is consulted only when nothing is installed. **Ambiguity is a question, never a coin toss** — two runners with no majority in the test files, or a Node backend where the built-in runner and Jest are both reasonable, return `ask` with candidates, and dismissing the QuickPick cancels the scaffold rather than falling through to a default. **Unit and end-to-end are separate choices**, since collapsing them made "already uses Cypress" read as "needs no unit runner". **Forbidden installs are data** — the plan carries what must not be added and why, so the never-Jest-beside-Vitest rule is walkable by a test rather than a comment somebody remembers. Test-file ownership is **exclusive** (`cypress/e2e/x.cy.ts` matches Playwright's `e2e/` rule too, so independent per-framework counting gave Playwright a majority in a project with no Playwright tests). The syntax adapter renders a neutral spec per dialect — Chai has no `toBe`, and Jest and Cypress inject their globals so an import is an error rather than a style choice. Pure + unit-tested |
...
| `RegisterHandoff` | `src/core/registerHandoff.ts` | Turning a register finding into planned work. Three registers record what somebody found — gap analysis, tech debt, risk — all three fed the score and the attention band, and none could become work; the Gap Analysis nav strip even said *"Turn a gap into planned work"* and routed to the Roadmap, where you retyped it with no link either way. **One module, not three**, because three rule tables would eventually disagree about how a finding becomes a sentence, and the symptom would be a backlog where a gap and a risk of equal severity read as different kinds of commitment. `RegisterFinding` is deliberately not any register's own record — each carries fields the hand-off does not want, and taking the whole record couples one module to three shapes that change for unrelated reasons. **No model in the path**: byte-identical output per finding, which is what makes it reviewable *and* what lets both actions show the exact text in a confirmation — you cannot show somebody what is about to be written if it is generated after they agree. **A prefix only where it changes what the sentence commits to** — `Close:` a gap, `Pay down:` debt (a shortcut that works, so "fix" would misstate it), `Mitigate:` a risk, the last matching `ideationDerivation`'s word and **pinned equal by test** so one backlog never carries two vocabularies. **Labels only from the declared taxonomy**, keyed on the *union* of the three category vocabularies since `security` means the same thing whichever register raised it; an unmatched intent is in `droppedLabels` **and** stated in the issue body. **Outstanding-ness is deliberately not decided here** — only the register knows what its own words mean, since an `accepted` risk is a closed decision while `accepted` debt is work somebody agreed to carry; `collectRegisterFindings` makes that call per register and it is pinned by test, because getting it wrong is silent. Provenance lives on the **roadmap** side (`RoadmapNodeRecord.origin`): the gap analysis is regenerated wholesale each run, so a link written there dies on the next scan, while a roadmap node has a durable id and survives a rename. The duplicate check runs *before* the dialog, since offering to add a duplicate and only saying so afterwards is how a backlog acquires the same item three times. Pure + unit-tested |
| `RoadmapGraph` | `src/core/roadmapGraph.ts` | The roadmap as a graph — what has to happen before what. An ordered list can only say *this one is more important*; it cannot say **this one cannot start until that one lands**, and a surface holding only the first quietly presents it as the second, which is how a backlog reads as well-sequenced and is still unbuildable in the order it is written. Five rules. **A declared edge always wins, and derivation may never contradict one** — somebody draws a link because they know something the text does not say, so derivation refuses rather than competes, and *says* it refused, since a suggestion silently withheld is indistinguishable from a rule that does not work. **A derived edge names the declared rule that produced it and is a suggestion until accepted** — auto-derivation is what makes the graph usable on a backlog nobody wired up by hand, but silently *writing* an inference would let a keyword coincidence reorder somebody's plan; suggestions stay in their own array all the way to the canvas, because merging them with declared edges destroys the only thing that distinguishes a decision from a match, and they move no column and block no node. Rule order *is* the policy: an item that *says* what it waits for outranks two that share vocabulary. `roadmapSubjectTokens` treats the focus classifier's own words (`security`, `refactor`, `docs`) as stopwords — they say what *kind* of work an item is and every security item shares them, so counting them as subject would link all of them. **Acyclic by construction, not by hope** — a cycle is a plan that cannot run, and a layout pass fed one loops or silently drops an edge; derivation seeds its adjacency from declared edges and grows it as it accepts its own suggestions, so neither a suggestion against a decision nor two suggestions against each other can close a loop, while a *declared* cycle is reported by name rather than broken, because the items in it are the finding (Kahn's algorithm, so a cycle is what is left over rather than something detected separately). **An estimate comes from a published table, never a model** — a number produced in March is not comparable with one produced in July, and comparability is the entire point; the rule travels with the number, and the AI-assistance toggle is **per node**, since "port the CSS" and "design the migration ladder" are not helped by the same amount (`undefined` never chosen, `false` a decision). **Absent is not zero** — no deadline is `no-deadline` never "0 days left"; `2026-02-31` is refused rather than rolled into March; `at-risk` is decided by the node's estimate *plus every outstanding prerequisite's*, since a two-day task due in three reads comfortable until you see the five-day task it waits on. `roadmapRouteTo` is **ancestors only** — dependents are what a node *unlocks*, a different question, and folding them in would make "the route to X" include work after X. Completed prerequisites are **kept**: the plan shows how you got here. A completed item leaves the plan unless something outstanding still needs it, in which case it stays as the left-hand end of a route somebody is walking — but is still *recorded* as delivered, since a record omitting the load-bearing half is a strange record. The completion canvas columns by **month**, not depth (it answers "when did this land", and depth would re-tell the plan), with undated work last, because an unknown date is not the beginning of time. A stored position always wins: dragging is a statement, and a layout pass overriding it would undo the user's work on every refresh. **Orientation is one placement rule with its axes swapped**, never two layouts that could drift, and it is *carried on the graph* rather than inferred from coordinates — "the trailing face of a node" is a different side each way round, and guessing would be wrong for exactly the nodes somebody dragged. Every layout constant is a multiple of `ROADMAP_GRID_SIZE` (pinned by test), or turning the canvas's snap on and then auto-aligning would leave every node a few pixels off the grid it claims to be on and the next drag would jump. Pure, clock-injected + unit-tested |
| `RoadmapGraphStore` | `src/core/roadmapGraphStore.ts` | Where the graph lives: `roadmap/roadmap-graph.json` + a `roadmap-graph.md` mirror. `improvement-plan.md` is still the one file that says what the work *is*; this is only ever an **overlay** — a record whose item left the backlog is reconciled away, since anything else would leave a deleted line on the canvas, the worst possible disagreement between two views of one roadmap. **Identity is durable and carried by the markdown** — item ids were positional (`roadmap-3`) and renumbered on every insert, which is fine for a list and useless for a graph; the line now carries an `<!-- rm:id -->` anchor, invisible in every renderer and still greppable. Minting is text-plus-**ordinal**, never a timestamp or a random value, because the roadmap is committed and two developers wiring up the same backlog must not produce a diff — and only items that actually gained graph data get one, so an untouched backlog stays clean. **A record whose anchor was lost is repaired, not duplicated**: anchored items resolve first, then unclaimed records are adopted by normalized text, and the ordering is what stops a duplicated line stealing another item's history. Reconciliation **never mints**, because it runs on every render and a render must not write. **Reads never throw and never seed over a newer file** (`interpretVersionedDocument`, kind `roadmap-graph`). A stored edge is normalised to `declared` — a suggestion is never persisted — while a **reversed pair is kept**, since two people declaring opposite orders is a real disagreement, reported as a cycle rather than resolved on their behalf. Branch names are **validated, not cleaned**: the value reaches `git`, and a nearly-valid name made plausible fails later at `git checkout` where nobody connects it back to the card. `dismissed` is stored as a *rejection*, not a reversed edge — "these two are unrelated" and "the other one comes first" are different statements and only the second belongs in the plan. `layoutOrientation` lives here rather than in a setting, since which way a graph reads best depends on its shape rather than on who is looking; snap-to-grid does not, since it only changes where *your* next drag lands. **Auto-align releases positions, never writes them** — writing coordinates would freeze one arrangement into a committed file and the next item added would land in whatever gap was left; every position is cleared including nodes a filter is hiding, since a half-aligned plan nobody can see is worse than an unaligned one. **Bulk-accepting the inferred tree is confirmed separately**: the suggestions are not new, the whole-board accept is, and forty inferences at once is exactly when a keyword coincidence enters a plan unnoticed — applied one at a time against a growing edge set so one that only turns circular because of another in the same batch is skipped and reported. `fs`-only + unit-tested |
...
| Roadmap dependency canvas | `src/views/projectDashboardPanel.ts` (`roadmap` page) + `media/projectDashboard.js` | The plan as a graph, behind a three-way selector (canvas / prioritised backlog / delivered) on the page that already owned the backlog. The list is untouched and still the only place drag-reorder sets next-work weighting — the two answer different questions and neither replaces the other. Every mutation names a node by an **opaque durable id and nothing else**: the host re-reads the roadmap and validates the id against it, so a crafted message can address a node that exists and can never supply the text, the position rules, or the rule that provenance is recorded from. The route filter, panning, zoom and the view switch are **offline** — every route is computed host-side and shipped with the snapshot, because a way of *looking* at a plan must not be something that can fail. A cycle is refused at the point of drawing rather than saved and reported, since an unexecutable plan is not a state worth persisting to show somebody. Urgency is a left border, not a filled background: six saturated cards read as an alarm even when three say "not assessed". Pointer events, not drag-and-drop — a drag redraws the node's edges continuously, and drag-and-drop only reports a drop. Four arranging controls, split by what they change: **Fit all** and **snap to grid** are pure viewing (no message, snap remembered per editor, fit measured from the real frame and never zooming past 100% to fill space); **auto align** sends a *direction and nothing else*, since a webview supplying coordinates would be doing the layout and two layouts would drift; **Calculate tree** carries the AtlasMind mark because it is the one control here where AtlasMind concludes rather than reports, and it only asks — the confirmation and every write live in the host. The canvas block sits *after* `renderRoadmap` in the webview script on purpose: `workflowSurface.test.ts` slices `renderWorkflow`→`renderRoadmap` to assert the Workflow page makes no network call, so code between those markers is swept into that page's assertions |
