...
Layout is deterministic (depth for the column, priority for the row) and a **stored position always wins**: dragging a node is a statement about how you read the plan, and a layout pass that overrode it would undo the user's work on every refresh.
