...
Dashboard load runs the same planner as a bounded local drift gate, but not the same permissive parser as an explicit import. It searches only markdown whose filename contains `roadmap` or which lives under a `roadmap/` directory, excludes the configured AtlasMind SSOT, dependencies, Git internals, and generated agent worktrees, caps files and bytes, and makes no network request. `parseAutomaticRoadmapItems` refuses `# Plan: ...` documents and the Objective/Context/Approach/Steps/Verification scaffold, then skips rows under validation-only headings such as Acceptance criteria and Definition of done. Those are execution evidence for a parent item, not independently prioritised work. The manual Markdown import stays broad because selecting a glob is an explicit intent signal. Adds, adoptions, renames, and checkbox changes are shown in a sectioned modal before `applyRoadmapImport` writes; conflicts and missing source items are only reported. The pass runs once per panel after durable anchors are ensured, so it cannot race a roadmap click or turn a render into a repeated writer.
