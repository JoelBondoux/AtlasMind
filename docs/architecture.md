...
On first open the dashboard seeds a professional pipeline from the repository's branch layout via `ensureSeeded` (detected production branch, `develop` as staging). Each `DeploymentStage` carries a plain-English `description`, a branch ref, config-source **location** (never secret values), hosting + health-check URLs, a data repository, and explicit `backupPolicy` / `promotionPolicy` / `rollbackPolicy`. Safety-first defaults are baked into the seed: production is `isProtected` and requires approval, and a data-bearing target with no backup command stays **deny-by-default blocked** from being promoted to. Per-stage status (the deployed version) is read from each branch's `package.json`.
