...
`src/core/lensDeclarations.ts` is the shared read-only status layer for those two declaration-backed views. It reports each exact root-relative file as missing, a valid empty starter, ready, invalid, unreadable, or unavailable and supplies only semantics-free `{ version, collection: [] }` starter content. `src/views/lensDeclarationSetup.ts` owns the installed setup command and the create boundary: disk-backed local and remote extension hosts use an exclusive `wx` write, so an existing file wins even if it appears after status collection, and unsupported virtual storage fails rather than weakening create-only behavior. The same command is reached from Getting Started, Settings → Project Runs, Project Dashboard Overview, and the State/Configuration missing-file notifications. Settings and Dashboard recompute status without creating anything merely because a panel rendered.

`src/core/lensChangeStory.ts` is the first Change Story/PR adapter, scoped to committed local Git evidence. `reviewWorkspaceChangeStory` requires the selected workspace folder to equal the repository root, lists Git-reported local/remote refs for explicit base selection, resolves the merge-base, and invokes only fixed read-only `rev-parse`, `symbolic-ref`, `for-each-ref`, `merge-base`, `log`, `diff --name-status`, and `status` arguments through `execFile`—never a shell. NUL-delimited commits and paths are bounded and normalized before `buildLensChangeStory` groups top-level components and path-signalled source/test/contract-schema/config/migration/docs/automation categories. `LensChangeStoryPanel` renders after ready; existing path actions use host-held ids and live root revalidation, while deleted paths intentionally have no Open/Ask target. The slice reads no patch contents, PR metadata, issues, reviews, CI, checks, network, or runtime evidence; dirty worktree state is named but excluded, path categories are not semantic impact, and the Git diff remains authoritative.
