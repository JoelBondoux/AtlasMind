...
Two surfaces call it. **Settings → Buzz** (the `buzz` page in `src/views/settingsPanel.ts`) lists the current bindings and any rejected ones, alongside every `atlasmind.buzz.*` switch grouped as Connection / Inbound / Persistence / Routing; because the gates are nested, a control whose parent switch is off renders dimmed and disabled while still showing its stored value — an inert setting is shown as inert, not as absent. **Project Dashboard → Director** offers the binding per person: give a contact the `buzz` channel and the "Add / Edit person" form reveals an AtlasMind agent picker, sent from `agentChoices` in the snapshot so the client never guesses an agent id. `ProjectDashboardPanel.handleSetBuzzAgentBinding` additionally rejects an agent id with no matching agent, so a rename cannot leave a binding pointing at nothing. The binding posts as its own message rather than riding on `saveDirectorConfig`: it belongs in settings, not in git-tracked project memory, and a refused binding must not block saving the person.
