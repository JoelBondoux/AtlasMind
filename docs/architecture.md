...
Project Dashboard prompt handoffs are resolved in the extension host. The registered per-workspace configuration value has first priority; a validated workspace-state fallback covers the split-brain development case where the current extension code is running but the outer VS Code window has not registered its new `package.json` setting; the manifest default applies last. Settings and Dashboard use the same resolver, so a destination shown as OpenAI Codex cannot silently execute AtlasMind's default route. Installed destinations are still re-resolved and validated at dispatch time. Participant contributions are reduced to one primary service route per extension; internal participant IDs remain aliases, while declared chat sessions remain distinct. Chat settings renders the shared Atlas prompt button as both the visual reference and the live test control. The test message carries only a validated destination id; the host supplies the fixed test prompt and constructs the same closed command shapes used by Dashboard dispatch.

The Dashboard **Editions** page models the public product that is being designed, separately from the
Versions page's observed release history. `src/core/releaseMatrix.ts` owns the pure v1 schema,
sanitization, bounded mutations, roadmap entity keys, and decision/readiness metrics for offering
columns, feature rows, and their entitlement cells. Missing cells remain unknown; only an explicit
...
never a path or Issue number supplied by the browser. Roadmap actions follow the same rule: the host
rebuilds item text from its current document, mints/uses a durable roadmap node id, and confirms deletion
from the roadmap separately from deletion from the matrix. Unlinking an Issue changes only the matrix
relationship.

The Dashboard Versions page's public-version portfolio is assembled at that same host boundary. It is a dedicated **Ship & record** page, linked from Release, rather than content below the release-readiness and delivery-performance surface. `buildPublicReleasePortfolio` joins the explicitly refreshed GitHub release records to the already-derived roadmap gates, gate routes, and graph plan records. A missing gate is an absent progress reading, never 0%; drafts are excluded from the public portfolio and previews remain identified separately from stable deployments. The browser can name a tag or roadmap node only. The host resolves that tag against the last rendered portfolio before opening the GitHub release, confirming an empty matching gate write, or constructing the evidence-labelled AtlasMind review prompt. This keeps repository URLs, tracked-file writes, plan paths, and prompt text out of browser control.
...
The VS Code tool-approval gate delegates runtime scope to `ToolApprovalManager`. A card decision and its
Bypass/Autopilot state change are atomic: the manager records the scope, removes every concurrent pending
request that the scope may cover, and only then resolves their promises. Bulk resolution re-checks the
non-waivable `toolBypassCeiling` and each card's declared `allowedDecisions`; protected remote actions and
restricted generated-skill reviews therefore remain pending. Resolvers are installed before listeners see
a card, so even a synchronous host answer cannot become a lost click. Ceilinged cards carry the policy's
reason into chat instead of looking like Autopilot was ignored.
...
**Failover never replays a side effect.** An attempt is the whole agentic loop, tools included, so failing over re-runs the task from the top on another model. That is harmless for reads and wrong for a commit or a push — "commit, push and promote" was observed being handed to model after model, each starting over on a repository the previous one had already changed. Each attempt therefore records in `TaskAttemptContext.sideEffectsStarted` every tool call it *started* that `isSideEffectingToolCall` says can change something: only the `read`, `git-read`, `network-read` and `terminal-read` categories from `classifyToolInvocation` are safe, and anything unclassifiable counts as side-effecting. If the attempt then fails, `describeReplayHazard` stops the turn with a reply naming what had started and advising a look at `git status` / `git log -3`, instead of failing over. A delegated-tool ACP attempt is treated the same way once its prompt was in flight (the failure names `session/prompt`, the enclosing provider timer fired, or the agent returned an error), because its tools run out of sight and AtlasMind cannot tell which ran; a failure before the prompt — spawn, handshake, sign-in — changed nothing and fails over as usual. Escalation to a stronger model is skipped on the same condition, since an escalation is a replay too.

An ACP agent that stops answering mid-prompt (`did not answer`, `went silent`, `was still working`) blocks its endpoint scope for the rest of the turn. Every model behind that agent runs in the same process, and the observed failure was four Codex models in a row, each waiting out the full timeout.
...
Session follow-through is representation-independent. `selectTaskScopedSkills()` reads both the legacy session string and the goal, summary, decisions, and open threads in `SessionContextBundle`. When a direct Git instruction follows an unfinished workspace mutation, that signal selects the bounded workspace tools alongside Git commit/push; otherwise the model could publish but could not finish the source change the session told it to complete. This remains selection, not authority: the agent eligibility pool, turn capability envelope, TDD gate, and per-call approval still apply.
...
The `session/prompt` budget (`ACP_REQUEST_TIMEOUT_MS`, 180 s) is an **inactivity** timeout, not a wall-clock one: every `session/update` notification and every agent request (a permission request, say) restarts it, and `ACP_PROMPT_CEILING_MS` (30 minutes) bounds the whole prompt. A fixed 180 s declared a Codex agent hung while it was running a commit hook, the tests and a push and streaming progress the entire time. `ACP_PROVIDER_TIMEOUT_MS` is derived as `ACP_PROMPT_CEILING_MS + ACP_HANDSHAKE_HEADROOM_MS` so the enclosing timer still cannot fire first. On a prompt timeout the adapter sends `session/cancel` before discarding the session, so the agent stops working in the background; the error says which limit fired ("went silent for Ns during session/prompt" or "was still working … after 30 minutes").
...
### WorkspaceBoundary (`src/core/workspaceBoundary.ts`)

The check every skill read and write passes through, and the observation that it was answering two questions with one root. `assertInsideWorkspace` used the open workspace folder both to *resolve* a relative path and to *contain* it. That is correct while every subtask shares one tree, and it is exactly what has to come apart for worktree isolation: a subtask running in its own worktree must resolve a workspace-relative path inside that worktree while still being unable to reach outside the workspace.

**An absolute path into the containment root is re-rooted, not obeyed.** A relative path follows `resolveFrom` for free; an absolute one does not, and a subtask handed an absolute path naming the *main* tree's copy of a file — from a dependency's output, say — would write straight back into the tree it was isolated from, which is the race arriving by the one route the resolution root does not cover. Re-rooting preserves what the path meant (*the file at that position in the repository*) and changes which copy it names, so a path naming a sibling worktree lands somewhere that does not exist rather than reaching another subtask's tree. The rule **cannot fire for a caller that passes one root as both**, since "inside `containWithin` but outside `resolveFrom`" is empty when they are equal — asserted by test rather than promised — and a path outside the workspace is left exactly as it came so the refusal names what the caller asked for.

**Resolution moves; containment does not.** `resolveFrom` is a parameter, `containWithin` is always the workspace folder. Worktrees live under `.git/atlasmind-worktrees` — *inside* the workspace — so an isolated subtask is contained by exactly the same rule as an ordinary one and **isolation needs no widening of what a skill may touch**. That was verified empirically before the design was fixed: `git worktree add --detach .git/… HEAD` succeeds, the files check out, and the parent's `git status` stays clean. A single "root" parameter would have let a caller move the boundary by accident while meaning only to move the resolution, which is why there are two.

Symlinks are resolved *before* the comparison, so a link inside the workspace cannot tunnel a read or write to a target outside it — including a path through a symlinked directory to a file that does not exist yet, which is why a not-yet-existing target is canonicalised by prefix rather than skipped. `realpath` is injected so the escape cases are tested without creating one on disk. Behaviour is preserved exactly from the inline version it replaced, down to throwing rather than returning when nothing on the path exists: the containment check would have rejected that too, but by a different rule, and a boundary whose reason changes under refactoring is one nobody can reason about.

### WorktreeIsolation / WorktreeManager (`src/core/worktreeIsolation.ts`, `src/core/worktreeManager.ts`)

Where parallel subtasks run, and what stops them writing over each other. `taskScheduler` runs up to five subtasks at once (`chunkArray(batch, 5)` into `Promise.all`) against **one working tree**, with no lock, queue or serialisation anywhere in the write path — two independent subtasks editing the same file is a read-modify-write race whose loser vanishes silently, with both reported as completed.

**The obvious remedy does not fit every subtask, and the policy says so rather than pretending.** A git worktree is a checkout of *tracked files*: no `node_modules`, no build output, no untracked state. A subtask carrying `test-run` or `terminal-run` would land somewhere its own tools cannot run, and "the tests failed" would be a fact about the isolation rather than the code. Installing dependencies per subtask is minutes and gigabytes, five times over, for a batch that may take seconds. So the plan has three placements: `isolated` (writes, needs only tracked files — gets a worktree, keeps its parallelism), `exclusive` (writes *and* needs the real tree — runs alone), and `shared` (writes nothing — races with nobody).

**Serialising writers is not conditional on the feature.** With `atlasmind.execution.worktreeIsolation` off, every writer becomes `exclusive`. The race is a defect, and a setting that is off must not reintroduce it; turning isolation on buys back parallelism rather than making the run safe. `needs-working-tree` is checked *before* the setting, so a subtask that could never be isolated is not reported as blocked by a switch that would not help it.

Write-capability is a declared skill set rather than a reuse of `classifyToolInvocation`, and the difference is the question each answers: that one asks *should a human approve this* (risk), this one asks *can two of these collide* (the filesystem). `git-commit` is high risk and races with nothing two subtasks would both touch.

`WorktreeManager` is the git half, with the runner injected so both halves test without spawning anything. Four rules, each about not damaging a borrowed repository: **only worktrees this run created are removed** (checked against `git worktree list --porcelain` *and* the run's own registry — the rule `localModelArbiter` applies to models it did not load); **detached, never a branch**, since a linked worktree pins its branch and `git branch -d` then refuses, which is the mess `skills/gitWorktree` exists to clean up; **paths are derived, never accepted**, composed from run and subtask ids reduced to an identifier charset under `.git/atlasmind-worktrees` (inside `.git` so a half-finished subtask's files never appear in the operator's Quick Open); and **cleanup is best-effort and reported** — a worktree that will not remove is litter, not a reason to fail a run whose work succeeded. A failed *creation* returns `undefined` rather than throwing, so isolation degrades to running exclusively rather than becoming a new way for a run to die.

`worktreeMerge` is the half that decides whether the feature is worth having: isolation is only useful if the work comes back, and only *safe* if work that cannot come back cleanly is neither lost nor forced. It is **a patch, not a file copy** — `git diff` in the worktree, `git apply` in the main tree, so git decides whether the change still fits; copying files over would silently overwrite whatever the main tree had, which is the write race moved to the end of the run and made harder to notice. Patches are **applied one at a time**: the subtasks ran in parallel, their patches do not, because a race at merge time happens when the run already reports itself finished. **Never `--3way`**, which can leave conflict markers in a file and call that success — a subtask's work half-applied into a file nobody has read is worse than the same work sitting in a directory somebody can be told about; `git apply --check` runs first so a refusal happens before anything is written. **New files need `git add --intent-to-add`** before the diff, or a subtask that created one has it silently dropped and it looks like the model never made it. A conflicted worktree is **kept**, because removing it would destroy the only copy of work the operator has not seen, and the report names its path but never carries a patch body — a patch is workspace content and the report goes to an output channel.

The runner takes optional stdin so `git apply -` can be piped. Writing the patch to a temp file would put workspace content on disk *outside* the workspace, where none of this project's boundaries reach it and nothing cleans it up after a crash.

All pure or injected + unit-tested, and the whole path is verified against real git rather than inferred: a probe repository with three worktrees confirms that a new file survives the round trip, that a conflicting patch is refused with **no** markers written into the working tree, that its worktree is kept while the merged ones are removed, and that the parent repository's status shows only the merged changes.

### WorktreeRun (`src/core/worktreeRun.ts`)

The wiring, and its own module because wiring is exactly where a correct policy is routed around. Placement, worktree creation and merge-back are each right on their own; what can still be wrong is a wave that starts before the one it was told to wait for, a downgrade that leaves a writer in a parallel wave with no tree of its own, or a cleanup that removes the only copy of work nobody has seen. All state lives in the returned closure rather than on the Orchestrator, because two runs can overlap and a field would let one run's cleanup remove the other's trees.

Four rules. **Writers are serialised whether or not isolation is on** — with the setting off every writer is placed `exclusive` and gets a wave to itself, slower than before and no longer losing edits; the race was never the price of not having the feature. **A tree that could not be made costs parallelism, never separation**: a failed `worktree add` downgrades that subtask to `exclusive`, since running it in the shared tree beside the other writers would be the race arriving under the feature's own name. **Work comes back between waves, not at the end of the run** — a later batch may depend on an earlier subtask's edits, so a deferred merge would leave the dependency ordering honoured and meaningless. **Nothing holding work is removed**: a worktree whose patch would not apply is kept and named, and one stranded by a run that ended early is kept if it has changes and removed if it has none, so an aborted run neither loses edits nor litters `.git` with empty checkouts. An unreadable worktree counts as holding work, because keeping cannot destroy anything.

**The offer to switch it on is made once somebody has watched it cost them twice.** `serialisedWriterCount` counts only assignments carrying the `isolation-disabled` rule — never `needs-working-tree` or `isolation-unavailable`, which would be exclusive with the setting on too. Offering a switch that would not have changed the run somebody just watched is worse than saying nothing: they try it once and stop believing the advice. `worktreeRun` reports it at most once per run through `onSerialisedWriters` (a run is what somebody waited through; a batch is an implementation detail), and the host decides whether to interrupt — the first occurrence in a workspace is silent, because the progress line already explains it, and the offer lands from the second. It is never modal and never claims to rescue the run in flight, whose placement is already decided. Turning it on writes to *user* settings rather than the workspace's, since a workspace update lands in `.vscode/settings.json` — a tracked file in plenty of repositories, and a personal speed preference should not produce a diff for somebody to review.

This is also why `placeSubTask` checks `gitAvailable` **before** the setting, alongside `needsRealWorkingTree`. All three orderings give the same placement and different explanations, and with isolation off in a repository that cannot make worktrees, "isolation is switched off" points at a switch that would not have helped. A placement that is right with an explanation that is wrong is the harder failure to notice, which is why the order is asserted rather than assumed.

**Neither hook throws.** Both run from a `finally` — the scheduler's, and the Orchestrator's around the whole run — so a throw would *replace* whatever ended the run: a billing abort would surface as a git error and the real reason would be gone. A merge that fails is reported and its worktrees stay, which is the same outcome as a patch that would not apply.

`TaskScheduler` grew two seams for it. `partitionBatch` splits one dependency batch into the groups it should run as — everything in a batch is *free* to run at once, which is not the same as safe, and the scheduler had no way to say so; it is async because deciding may require creating something first. `afterBatch` runs after every chunk including one that threw, since the run that aborted is the one with work somebody needs to recover. With no partitioner supplied the chunking and the batch totals are byte-for-byte what they were, which is asserted rather than assumed: a total that became an estimate for every run, to serve a feature that is off by default, would be a worse trade than the feature is worth.

The Orchestrator applies it to both fan-out paths — `processProject` and `processTaskMultiStep` — because a multi-step chat turn loses writes the same way, and fixing the race on one surface only would mean it depended on which surface started the work. An isolated subtask's worktree travels to the tool loop as a **path**, not a context: the derived `SkillExecutionContext` is built where the tools actually run, so nothing in between can hold a live object that outlives the subtask. Post-tool verification is **skipped** for an isolated subtask — the verifier reads the editor's diagnostics, which describe the workspace copy of a file the subtask never touched, and "no problems" about the wrong file is a pass nobody earned.

`SkillExecutionContext.withResolutionRoot` is how the host provides this, and it is optional: a host that cannot re-root file access cannot isolate, which the placement already treats the same as having no git. The VS Code implementation rebuilds the context with a different `resolveFrom`, so every path-taking method moves together rather than some of them — a context whose `writeFile` went to the worktree and whose `applyPatch` went to the main tree would be worse than no isolation. Delegation (`runAgent`) is copied onto the derived context, since the Orchestrator installs it on the base context after construction and a context derived later would otherwise be the one agent in the run unable to hand off.

Git for this path is an `OrchestratorHooks.runGit` hook rather than `SkillExecutionContext.runCommand`: it needs stdin, which `git apply -` requires and no skill has ever wanted, and it is not a tool — every command it runs is a constant in `worktreeManager` or `worktreeMerge`, never composed from a model's output. The hook declares `encoding: 'buffer'`, which is load-bearing twice: without it the child streams emit strings and `Buffer.concat` throws on the first call — indistinguishable, from the outside, from a repository that cannot make worktrees — and a patch is decoded once at the end rather than per chunk, so a multi-byte character split across a chunk boundary survives.
...
**A reopened chat adopts the run rather than watching it from the status bar.** The panel already adopted runs started in another *open* surface — busy state and the stop button both resolve through one lookup across every live panel — and a detached run was invisible to that for exactly one reason: its panel had left the live set. `ChatPanel.detachedPanels` is that set's counterpart, collected for busy state and stopping but *not* for syncing, since a detached panel has nothing to draw to. `selectBusyRun` declares the order: **this session first**, because a surface must not report work from a conversation it is not showing, then **live before detached**, because two runs can share a session — close a chat mid-answer, reopen it, ask something else — and the one just started is the one being watched while the other is already named in the status bar. Ties keep arrival order so the choice cannot shuffle between two identical renders, and with nothing detached the ordering agrees exactly with the rule it replaced, which is asserted rather than assumed.

Streaming reaches the reopened chat because a detached panel's coalesced tick pushes to whatever surfaces are open instead of to its own. It stays coalesced and passes `reuseProviderList` — enumerating providers touches credential storage, and a per-chunk tick must not. The `sessionConflict` guard counts detached runs too: asking something else in a chat reopened onto a session a background run is still writing would otherwise interleave two answers into one transcript.

The other panel's `streamingThought` and model chips are read from the surface that owns the run, but **only for the session on screen** — a run on another session must not lend this one its thoughts, and the fallback to another session's execution exists only to supply a streaming target.
...
- **`uiSurfaceScan.ts`** — which files in the workspace are UI, and which adapter would read each. `uiRepositoryMapping.ts` validates a source path it is *given*; nothing scanned for one, so every mapping began with somebody typing a workspace-relative path from memory — fine on a project you wrote last week, useless on the one you have just been handed, which is the case the Studio exists for. Five declared rules classify by extension and location, confirming with a bounded read of a file's head only where the path cannot answer: a stylesheet qualifies only if it declares custom properties, and a `media/` script only if it builds markup, or each rule would claim every reset and polyfill beside it. Each candidate names the rule that claimed it and the table travels in the report, so the list a person picks from is arguable rather than taken on trust. Dependencies and build output are never entered — `out/`, `dist/` and `coverage/` matter most, holding *derived* copies of the project's own UI, so they look right and a mapping onto one records a source the next build overwrites. Bounded on directories, files and results with the truncation stated, and total: an unreadable directory is a miss rather than a failure, since under-reporting is safe when anything missed can still be typed by hand. The Studio renders the result as a `<datalist>` rather than a `<select>` for that same reason — a menu would make the scan's misses unreachable instead of merely unlisted — and distinguishes an empty result from an absent scan. Pure apart from the injected reader + unit-tested; verified against this repository at 7 surfaces from 1,456 files with no false positives.
...
### DirectorPriority (`src/core/directorPriority.ts`)

Two different questions asked of one Project Director config, after the sidebar spent several releases asking one question twice.

`Project State` says *"waiting on you"*; `Project Director` says *"the project"*. Both called one collector — every due follow-up plus every assignment owned by the configured self contact — so on a project where the director and the developer are the same person the two badges carried the same number for the same reason, and the second one carried no information at all.

`collectSelfWork` answers the first: **what is mine**, scoped by name. `buildDirectorPriorities` answers the second: **what should the project do first, and what is somebody else sitting on**, as a ranked board.

Six rules keep the second honest, and each is a rule this codebase already applies elsewhere:

1. **Mine means named.** An unowned record falls to the self contact only on a *solo* project. On a team project an unowned item is nobody's — a finding for the Director board rather than a row on somebody's personal list, because putting it there would quietly assign it.
2. **Rank from a declared table, never a model.** `DIRECTOR_PRIORITY_RULES` grades every row and the row carries the rule that graded it, so a grade produced in March is comparable with one produced in July and can be argued with in either.
3. **Ranked by consequence, not magnitude.** Declaration order *is* the ranking: work three other items wait on outranks a pile of individually late ones. Ties break on the same order, so the list cannot shuffle between renders.
4. **An item is graded once,** by the first rule that matches, so a blocked, overdue, stalled assignment appears at its worst grade rather than three times.
5. **Unassessed is not clear.** Dependency information is optional and *absent* means the question was never asked — never "nothing is blocking anything". The tree renders that as its own row, and only when at least one active item links to work a dependency graph could have said something about.
6. **Capped, with the remainder stated.**

"Holding up other work" is derived from the only dependency information AtlasMind actually holds: the roadmap graph's **declared** edges, where `from` has to land before `to` can. Suggested edges are excluded — a keyword coincidence must not tell somebody their colleague is holding up the release — and a dependent counts as still waiting unless its record carries a `completedAt`, because only a recorded completion is evidence that it landed. The tree reads that file at most every 30 seconds and treats an unreadable graph as *not assessed*.

`roadmapDependentsForAssignments` carries the awkward half: **the two sides do not share an id.** A Director assignment on a roadmap item carries the *positional* `roadmap-3` every dashboard surface keys on, while the graph is keyed on the durable anchor id. Joining on id alone would match nothing and the blocking rule would quietly never fire — which is worse than the rule being absent, because the board would still read as having looked. The join therefore goes through the item **text**, which both sides hold: the assignment's title is the roadmap line it was created from, and the node record stores that line's normalized form. It is the same repair path `roadmapGraphStore` uses when an anchor is lost, and it is deliberately biased to false negatives — a renamed item stops matching and is simply not flagged, rather than adopting whatever text took its place.

"Taking too long" is two separate measurements, not one: past a declared due date, and started-or-blocked with nothing recorded against it for `STALLED_AFTER_DAYS`. A record carrying no dates is **never** called stalled — an unknown age is not a long one, and "nothing recorded for N days" would be a claim about a measurement nobody took.

The Director badge counts `flaggedCount` — items graded `blocking` or `late` — and never the ready-to-pick-up rows beneath them. A badge that counted the backlog would be permanently non-zero, which is how a badge stops being read; the same reasoning that keeps a *classified* CI failure out of the Project State badge.

Pure and `vscode`-free, with the clock injected. `calendarDaysBetween` is exported from `projectDirectorManager` rather than reimplemented here, so one module decides what day it is and a follow-up cannot read overdue on one surface and due today on another.
...
The **Waiting on you** gather reads Project Director's in-memory source of truth through `collectSelfWork` and selects records that **name this person**: active assignments and due or overdue follow-ups whose owner is `selfContactId`; `done`, `cancelled` and colleague-owned records are excluded, and an unowned record counts only on a solo project, where there is nobody else it could belong to. Follow-ups were previously taken *whole* — every due follow-up in the project regardless of owner — which is why this view and Project Director reported the same number for the same reason on a project whose director is also its developer. Due follow-ups are projected individually rather than collapsed into a count. Each row carries an opening-only destination and a stable target id: dashboard work uses `ProjectDashboardOpenTarget`, runs use `ProjectRunCenterOpenTarget.runId`, and manual assignments/follow-ups point to their exact Director record. VS Code's `TreeView.badge` is container activity—it decorates the AtlasMind activity-bar icon but not a native view header—and VS Code also hides `TreeView.description` when the view collapses. The one count is therefore projected through three APIs: `TreeView.badge` for the container, a dynamic `TreeView.title` (`Project State · N waiting`) that survives both expanded and collapsed states, and a synthetic-URI `FileDecorationProvider` for the coloured numeric badge on **Waiting on you**. Project Director applies the identical three-channel pattern to its **Work on next** projection, over a different question — see `DirectorPriority`. Owner saves invoke the Project State refresh command immediately, and `projectDirectorRefresh` covers both trees and external file edits.
...
### AgentEvalHarness (`src/core/agentEvalHarness.ts`)

Golden tasks for an agent, and the regression check that stops a rewrite quietly making it worse. `agentAutoUpdater` asks a model to rewrite an agent's system prompt on a cadence and registers the result — a prompt edit, deployed automatically, with **no equivalent of a failing build**. That is precisely the failure this project's own Prompt Regression protocol names: *a wording change that fixes one case and breaks nine is invisible without a replay set.* The cadence is the risk; this is the replay set. (`modelEvalHarness` answers a different question — which *model* suits one prompt — and scores by completion quality.)

**A case is graded by a declared check wherever possible, and every result says which graded it.** `must-contain`, `must-not-contain`, `must-match` and `must-refuse` are decided by reading the text; `judge` exists for what genuinely cannot be and is marked on every result it touches, because a model grading a model is the weakest link in any eval and a suite that conflated the two would report the strength of the weaker. A judge check with **no verdict fails**: a suite must not go green by not asking. A `must-match` pattern that will not compile fails rather than passing, since a broken case needs attention and passing it hides whatever it pinned.

**An errored case is not a failed case** — grading short-circuits before any check is looked at, because an empty output would fail every `must-contain` and make an outage look like a broken prompt. Errored cases are excluded from the verdict and named, or the gate blocks every rewrite during a bad afternoon and is switched off by Friday.

**A regression is `pass → fail`, never a score that moved**; scores wobble between runs on identical input. **No baseline is a first run, not a pass** (`observedDelta`'s rule, and it matters more here because the first run is when somebody decides whether to trust the harness). **An unrun case is not a passing case**, so coverage travels with the verdict. And a run records a `definitionFingerprint`, without which a baseline says "this agent passed" rather than "this *version* passed" and a rewrite would inherit the previous version's results.

`shouldHoldAgentUpdate` is where the harness meets the risk. Three answers, and the middle one carries it: an update that **could not be verified** is held rather than allowed, because the cadence is unattended and "we did not check" resolving to "ship it" is how an unattended rewrite becomes an unattended regression. An agent with **no declared cases** is not held — that would freeze every agent on day one, which is how a gate gets turned off — but the decision says plainly that nothing verified it.

In `agentAutoUpdater` the gate runs *before* `register`, skips a run that produced no change, treats a throwing verifier as unverified, and keeps `heldUpdate(agentId)` so a surface can say why an agent stopped updating — an update that silently stops is indistinguishable from one that never came due. `agentEvalRunner` fills the seam: it replays against the **candidate** definition rather than the live one, at temperature zero so sampling noise cannot read as a regression, and recording a baseline is a separate confirmation because overwriting one is how a real regression becomes the new normal.
...
### UiSurfaceEmit (`src/core/uiSurfaceEmit.ts`)

Emit a designed surface for the engine that will own it, and keep the words editable from here afterwards. The rule the module turns on is **divergence, not regeneration**: a renderer that regenerated the target from the graph on every save would overwrite the first engine edit, and forbidding engine edits makes the emit worthless. So the layout is emitted once — Web (HTML + CSS), Unity UI Toolkit (UXML + USS), Godot 4 (Control scene + Theme), or a handoff specification for Unreal UMG, SwiftUI and Compose whose syntax was not verified — and belongs to the engine from then on. Every emit records a `UiEmitManifest` (`project_memory/domain/ui-emit/<screen>--<target>.json`) with a fingerprint per file and a `UiEmitAnchor` per node; `planSurfaceEmit` refuses a second emit over files whose fingerprints changed (`layout-owned-by-engine`), files at the target paths it did not write (`files-exist`), a surface nobody drew, and a folder outside the workspace or inside `project_memory/`. The words stay Studio's because they are emitted as data inside a region keyed by the node id — an element name in HTML and UXML, a `[node name="atlas_copy_<id>…"]` block in a `.tscn`, a comment marker only in the specifications, since Unity's UI Builder and Godot's editor re-serialise their own files and drop comments. `planContentPatch` locates each region by anchor in the file *as it is now* and produces one slice replacement per changed region, refusing by name a missing anchor, an ambiguous one, a region whose fingerprint shows a hand edit (with the current text carried on the refusal), a node removed in Studio (never deleted from the engine file) and reporting nodes drawn after the emit as unanchored (never inserted). `assessSurfaceOwnership` reads the files as they are and composes the statement — *Layout: owned by Godot since the emit on … · Content: editable here (2 of 3 regions; 1 blocked)*. `planSurfaceLaunch` fills two placeholders in a per-target constant argv; the module imports neither `fs` nor `child_process`. Pure + unit-tested.

### BrandPresets (`src/core/brandPresets.ts`)

One named set of design decisions, applied to many surfaces. UI Studio held its visual decisions in two places that could not agree — a flat `WebsiteDesignSystem` labelled *legacy* on its own page, and the typed token graph directly beneath it. Two sources of truth for "what colour is primary" is a parity bug in waiting, and it is why a brand had nowhere to live: there was no object that *was* the brand, only two partial descriptions of one. A preset is that object — a small set of **value** tokens keyed by a declared role from `BRAND_ROLES` (`color-primary`, `font-heading`, `spacing-base`, `radius-base` …), the same ids `websiteWireframePreview` already resolves, so nothing downstream learns a new vocabulary.

Six rules. **Applied by alias, never by copy**: `applyBrandPresets` materialises every preset's tokens into the graph as `brand-<preset>-<role>` and points the role tokens at the default preset with `aliasOf`, so the existing `resolveUiDesignToken` reads straight through and a change to the preset reaches every surface; copying would leave twelve screens each holding a stale primary the day the brand changed. **A local override is a value, and it is reported** — a role token holding a direct value is left alone by application and named `overridden` by `describeBrandApplication`, since a surface that quietly kept its own primary while claiming the brand is the failure a preset exists to prevent. **Materialised tokens are a projection**, rebuilt from the presets on every sanitize and pruned when their preset goes — the graph's own idiom, where the wireframe is derived from the screen rather than asked to agree with it; a hand edit to one does not survive. **An extracted preset cites its source and invents nothing**: `extractBrandPresetFromStylesheet` reads `:root` custom properties through the published `BRAND_ROLE_NAME_RULES` (first match wins, deliberately literal), records the property and line for every role filled, and lists every property it could not use with the reason — a `var()` reference is not a value, a `rem` length would need the root size to convert, a name matching no rule, a second declaration for a filled role. Commented-out declarations are blanked before reading. **The legacy design system folds once, at migration, and only if it was changed**: `brandPresetFromDesignSystem` returns nothing for a system still at its seeded defaults, because a brand nobody chose must not be attributed to them; prose fields (`spacingScale`, a non-pixel `cornerStyle`) travel as notes rather than being guessed into numbers. **Nothing here writes anything.**

Preset tokens are direct values only — the sanitizer drops aliases, since a preset token aliasing another would make "what is primary" depend on something outside the preset, which is the two-sources problem back again — and the `brand-` prefix is refused as a preset id because it would collide with its own projection. A citation pointing outside the workspace is dropped rather than cleaned. In `websiteWorkspaceManager`, format 14 adds `brands` and `defaultBrandId`; on every sanitize the graph tokens are rebuilt around the presets and the legacy `designSystem` fields are **projected from the default preset** (`projectDesignSystemFromBrand`) so the readers that still consume them cannot disagree with the graph. `UiDesignScreen.brandRef` names a preset other than the default and is kept even when no preset carries the id — `resolveScreenBrand` reports the dangling reference rather than the sanitizer silently dropping it. Pure + unit-tested.

Format 14 also carries `WebsitePagePlan.source` (`WebsitePageSource`: `path`, `adapterId`, `ruleId`, `pickedUpAt`) — where a surface came from when it was picked up out of the project by the Studio's rail rather than drawn. Recorded at the moment of pick-up from the scan's own classification and never inferred afterwards (a page whose title happens to match a file is not that file); `sanitizePageSource` validates rather than cleans — traversal, an absolute path, an unknown adapter or an unreadable date drop the source whole — because the path is later resolved against the workspace and shown as a citation, and one repaired into a different file would cite the wrong one. `UiEmitTargetId`, `UiEmitAnchor` and `UiEmitManifest` (0.473.0) are the emit record: the anchor keeps its two fingerprints apart on purpose — the region as written and the copy it came from — because neither implies the other, and the manifest is committed so a teammate can see who owns a layout.

### ProjectBrief (`src/core/projectBrief.ts`)

The project in the user's own words, and what may honestly be derived from it. Every other starting point AtlasMind has is *detected* — the archetype from manifests, the templates from the archetype, the roadmap from whatever somebody types line by line — and none of them can know what the project is **for**, because that fact exists only in the head of the person starting it. So the board opened onto generic questions and the roadmap onto nothing at all.

The whole module is about one hazard. A model handed "a booking app for dog groomers" produces twelve confident cards about payment flows, cancellation policies and SMS reminders — none of which anybody said, all of which read afterwards like decisions somebody made, in a committed file where a later reader cannot tell them from the real ones. That is the `researchRegister` failure with a shorter fuse, and it is closed the same way: **in the sanitizer, not in the prompt.** A prompt is a request; a sanitizer is a guarantee.

Seven rules. **The brief is kept verbatim and never rewritten** — control characters go and the length is capped, and nothing else is touched, including spelling, capitalisation and paragraph breaks, because a tidied-up version is somebody else's words and the file exists so derived cards can be checked against what was actually written. **A derived card either quotes the brief or is a question**: `quote` must appear verbatim, checked here rather than trusted, and a card whose quote cannot be found is **demoted to a question, not dropped** — the model may have noticed something worth asking even having invented the supporting words, and a question commits nobody to anything. **The demotion count is always reported**, since a proposal that quietly discarded its ungrounded half would look like a model that got everything right. **A brief too thin to derive from is refused, never expanded** — the floor is low but real, and the refusal names the two facts that are missing rather than saying "too short".

**Three acts, not one**: writing the brief, seeding the board and raising roadmap items are separate confirmations, because each writes to a different committed file. **Nothing here writes anything** — it returns a document, a template and a proposal, so the board's own seeder and `addRoadmapItemFromExternalSurface` stay the only writers. **The brief is not a plan**: cards land as problems, requirements and experiments, never as conclusions with the argument already settled.

Quote matching normalises whitespace and case and deliberately **not** punctuation: a model that reproduced the words and changed the punctuation copied them, while one needing punctuation stripped to match is paraphrasing, which is the thing being checked for. `briefToBoardTemplate` emits **no edges** — an edge asserts that one card supports or contradicts another, a claim about the project the brief did not make, and inferring those would invent the reasoning as well as the content — and no positions, since layout belongs to the board. The quote travels onto the card body, so provenance survives where somebody will read it.

In the panel the reading is grounded against the **file**, not the message: the derive action carries no payload and the host re-reads the brief it wrote, because a webview supplying both the claim and the evidence for it would make the quote check meaningless. `seedBoardTemplate` was widened to take either a declared frame's id or a built template, so there is still exactly one thing that writes cards. Pure, clock-injected + unit-tested.
...
The other architecture packs are *project* templates: pick a stack, get a starter. These six are not that shape. Nobody starts a project called “payments”; they reach the point in an existing project where money has to change hands, and what they do next is pick a library and discover the decision afterwards. That is the wrong order and the expensive one — whether you or your vendor is the merchant of record is a tax-liability question no SDK swap undoes, and whether analytics sets a cookie decides whether a consent banner is owed at all. So a pack leads with the question and treats candidates as answers to it, with a test asserting every candidate answers one of its pack's declared options.

**Every command is a constant in this file, verified on a stated date, and nothing runs one.** `UTILITY_PACKS_VERIFIED_AT` pins when each vendor fact was read from that vendor's own documentation — the idiom `ACP_SPEC_VERIFIED_AT` and `BUZZ_PROTOCOL_VERIFIED_VERSION` already use, because a catalogue of somebody else's product decays and a reader deserves to know how old it is. A test walks every `install` for shell metacharacters, asserts each is a single-quoted literal with no interpolation, and asserts the module imports nothing that could execute one.

**An unverified fact is absent, not guessed.** A candidate whose install line was not verified carries none, and the surface says to follow the vendor's current instructions. A plausible-looking wrong package name is worse than a missing one: the first installs somebody else's code, the second costs a search. Documentation links are roots rather than deep links, pinned by a test, because a versioned guide URL is the first thing to 404.

**A capability already present is reported, never proposed again.** `assessUtilityPack` matches declared dependency names — never inferred source shape, for the reason `testingSubjects` refuses to — with the lookaround boundary `testingAutoAssess` uses, since this vocabulary is full of `@`, `/` and `-` and `` behaves surprisingly around all three (`stripe` must not match `stripe-mock`). The interesting result is **`ambiguous`**: two candidates answering *different* sides of one decision is not redundancy but a project with two session models, or one that is both the merchant of record and not, and the right response is to say so rather than propose a third.

**What leaves the machine is stated per candidate**, including where it is nothing — asserted by test, because “add analytics” means “start sending your users' behaviour to a third party” and a catalogue omitting that is selling rather than advising.

**Accessibility is not a library.** The one pack with `installable: false`, a flag rather than a special case at each call site so a surface cannot offer “add accessibility” as though it were a dependency. Automated tooling catches roughly 30–40% of WCAG barriers (Deque publish ~57% for axe-core on a first audit), which makes it necessary and not sufficient — the same argument the accessibility testing protocol already makes. The pack lists tools and then states the human half, with the EAA's 28 June 2025 application date and the EN 301 549 / WCAG 2.1 AA conformance route named.

**A gate is a statement about the world, not a file that exists.** “Somebody has completed an account recovery end to end from a real inbox” is a gate; “there is an `auth/` directory” is not, and a test asserts no gate mentions a file or a directory.

The packs render on the Gap Analysis page — the page that already asks what this project is missing — against the manifest corpus `collectArchetypeEvidence` already builds, so they add no I/O. With no manifest readable every pack reads `unassessed` rather than `absent`, and nothing is offered: an offer built on nothing observed is a confident zero wearing a suggestion's clothes. `buildUtilityDecisionPrompt` forbids the agent two things — installing anything, and stating a price, version or feature it was not given, since the list is a dated snapshot and vendors move faster than a release.

### AmbientTriggers (`src/core/ambientTriggers.ts`)

The event bus ambient agents subscribe to — what may wake AtlasMind up when nobody asked it anything. Everything else here begins with a person typing, which is the right default and was also the ceiling: a failing pipeline, a new advisory, a review sitting on somebody and a blocker defect all happen while they are looking elsewhere, and none of it reached them until the dashboard was next opened.

What "while you're away" can honestly mean is stated in the module rather than implied by the name. A VS Code extension does not run when the editor is closed, so this is *ambient* rather than *background*: it works while somebody is in the editor and not looking at AtlasMind, which is where nearly all of the time goes.

**An event is a change, not a state.** A pipeline that is red stays red; if "red" were the event it would fire on every evaluation forever, and the first thing anybody would do is disable it. An event is a *subject* entering a condition it was not in last time, fingerprinted `kind::subject`. A condition that clears is dropped from the seen-set so the same one returning later fires again — accumulating instead would mean a flaky pipeline fired exactly once, ever — while a kind that could not be observed keeps its memory, or an unreadable source would replay every standing condition the moment it came back.

**Deny by default, twice**: a master gate and a per-event subscription, both off, because switching the feature on and subscribing to a particular event are different decisions.

**Ambient never exceeds `propose`.** The rule that makes the rest safe. `resolveAmbientCeiling` takes the minimum of four things and the first is the constant `propose`: a workflow stage may permit `auto` because somebody started that run and is watching it, and nobody is watching this one. Two kinds are capped at `observe` *by declaration* rather than by configuration — `security-advisory` and `release-blocked` — because the useful unattended act there is to tell somebody, and a proposal generated unattended about another party's disclosure, or about a step that cannot be undone, is worse than the alert. Every reduction is named.

**A missed window is not a backlog** (`researchSchedule`'s rule, same reason), **unknown is not quiet** (an unreadable source is `notObserved`, never an empty list — the shape carries the distinction so a caller cannot report "nothing" for something it never looked at), and **spend defaults to nothing** (`researchSettings`' argument: switching triggers on and letting them cost money are two decisions). The spend cap deliberately bites only on a `propose`-level action: telling somebody what happened costs nothing, and refusing to because a budget ran out would be the worst possible reading of a cost control.

**Nothing here runs anything.** `planAmbientResponses` returns a plan, so a policy change cannot become an execution — the property `producerReportPublication` has, for the same reason. Suppressions are returned *alongside* the actions rather than filtered away, because a feature that silently does nothing is indistinguishable from one that is broken and the commonest cause of silence is a gate somebody meant to open.

`AmbientTriggerService` holds a timer and nothing else; the observation, gates, storage and presentation are injected, so every rule is testable without a running editor. It writes the seen-set **before** presenting and **even when the master gate is off**: a crash between the two would replay the event, and a gate that forgot would raise every standing condition at once the moment it was switched on — which is exactly the experience that gets a feature like this switched off again. The seen-set is per developer in workspace state, never in `project_memory/`, because a shared one would mean "has *anybody* been told" and the first person to open their editor would silence it for everybody else.

`buildAmbientHandoffPrompt` says two things an interactive prompt does not: that nobody is watching, and that **the trigger is not a brief** — an event says a condition exists, not what should be done about it, and an agent treating "CI failed" as authorisation to change the build would be acting on nobody's decision.

In `extension.ts` the observation is deliberately narrow: the defect, approval and test-case registers are local files and always readable, while CI, advisories, review requests and release gates need the network and are reported as `not-observed`. That is rule 5 doing real work rather than being asserted.

### TestCaseRegister (`src/core/testCaseRegister.ts`)

The other half of testing. Everything else AtlasMind knows about tests is derived from **files** — which methodologies are declared, whether anything evidences each, which declared subjects have a test that names them, how many cases the suite contains — and that is the half a machine can answer. It says nothing about the cases somebody wrote down, who owns them, when one was last actually carried out, or what a tester needs in front of them to do it. That gap is not a shortfall of the file-based reading: exploratory testing, an accessibility pass with a screen reader, a device matrix and a disaster-recovery rehearsal leave nothing for a scanner to grade, which is why this repository's own testing protocols name TestRail, Zephyr and Xray.

**Priority comes from a declared table and is never asked for.** `TEST_CASE_PRIORITY_RULES` grades two facts a person can actually state — what breaks if this is wrong, and how often the path is taken — in seven ordered rules; `gradeTestCase` is total by construction, walked by a test over the product of both enums. The reach-independent rule comes first for the reason `defectRegister` grades data loss the same way. The grade is recomputed on read, so a hand-edited priority in the committed file does not survive.

**A case that was not run is `not-run`, never `pass`.** There is no default result and no way to seed one; an unrecognised stored result coerces to `blocked`, because "we do not know that this works" is the safe direction, and an unrecognised status coerces to `draft` rather than `active` — a case is in a team's run set because somebody put it there.

**A result belongs to a revision of its case.** `reviseTestCase` bumps `revision` when the steps or the expected result change, and it does so *there* rather than leaving it to a caller, because a call site that forgot would silently keep an old pass counting for a rewritten test. `testCaseStanding` then reports `stale` rather than the result it recorded. The execution record is neither edited nor discarded: the register keeps what happened and stops claiming it describes the case as it stands. Without the revision, "was this case passed?" and "was *this version* of it passed?" are the same question and the answer is always yes.

**An automated case is never given a result here**, and `recordTestExecution` returns a typed refusal rather than throwing or silently doing nothing. Its result comes from the JUnit report `testingPolicyCoverage` already reads; a person recording a manual pass for it would be asserting what a machine should measure. A deprecated case is refused for the adjacent reason — somebody decided it is no longer worth running, and a result against it would revive it through the back door.

**A test asset names where a credential lives and never holds one.** The point of recording a tester's context is that it is shared with a named person, so a value stored there is a value distributed. `CREDENTIAL_SHAPES` matches on *shape* rather than on field name — the field somebody puts a password in is never the one called `password` — and a match **refuses the whole asset**, on write and again on read, rather than stripping the offending field: a silently scrubbed record reports success while the secret stays in whatever it was pasted from. An asset with no owner is *unassigned*, never everybody's, and an unowned case is reported as one nobody runs rather than folded into whichever owner has the most.

**Cases transition; nothing is deleted.** A vanished case is indistinguishable from one that never existed, and the results recorded against it become orphans — the sanitizer drops an execution whose case has gone for that reason, rather than counting a result for a test nobody can read.

Nothing here grades a methodology. `testingPolicyCoverage` still owns whether one is evidenced and it reads files; a manual case is additional evidence a person can point at, not a substitute. On the dashboard the register renders as a card on the Testing page rather than a page of its own, since the item asked to extend the test surface and the two halves belong beside each other. The webview posts what a case *is* and never a priority; `buildTestCaseDraftingPrompt` may propose steps and may not state a result, because a stated one is indistinguishable from a real one once it is in the register and somebody relies on it before a release.

### ChangeApprovals (`src/core/changeApprovals.ts`)

Who agreed to what, and to which version of it. AtlasMind already had approvals in two senses and neither is this one: a tool approval is a *permission*, asked and answered in seconds, authorising an action about to happen; a release gate is a *condition*, evaluated from evidence. What was missing is the durable record that a named person agreed to a change — an idea reaching the roadmap, a document going out, a licence term, a commercial commitment.

**An approval is not a permission.** Nothing here grants a capability, unlocks a branch, or lets a tool run, and nothing here blocks a commit or a release. A gate AtlasMind cannot enforce is a gate that teaches people to route around it; a record is worth having precisely because it outlives the conversation that produced it.

**Pending is not approved.** Absence of a decision is never read as consent. There is no auto-approve path and deliberately no timeout that grants one — "nobody objected within five days" is the single most common way an approval process comes to certify things nobody read. The sanitizer applies the same rule at the boundary: an unrecognised stored status coerces to `pending`, because the file is committed and hand-editable and an unknown value must never resolve in the reassuring direction.

**An approval names *what* was approved.** Every request carries a fingerprint of its subject's content, and an approval stamps the fingerprint it was made against. `approvalCurrency` answers four ways rather than two: `not-approved`, `current`, `stale`, and — the interesting one — `unresolvable`, when the subject can no longer be read at all. A deleted document or an item that left the roadmap leaves an approval nobody can check, and reporting that as current is the confident-zero this codebase keeps refusing; it is not `stale` either, because nothing is *known* to have changed. Staleness neither revokes the approval nor edits the decision: silently re-approving would forge a signature, silently rejecting would discard a decision somebody really made.

The content is **never stored**, only its fingerprint, because `approvals.json` is committed and mirroring a legal draft or a commercial term into it would put the very thing under review into the repository's history. A fingerprint is carried only on an *approval*: a rejection is a statement about the change rather than a signature on a version of it, so "stale rejection" is a state nobody needs.

**Nobody is substituted.** `APPROVAL_ROUTING_RULES` names a role per category — code to a reviewer because a review *is* the approval, documentation to a maintainer as day-to-day delivery, and roadmap, legal and commercial to the Director because nobody else can accept those on the project's behalf. The roster is searched in declaration order rather than by seniority, which is not modelled and would be guesswork. When nobody holds the role the result carries a written reason and **no approver**: falling back to the Director, or to the first name on the roster, would produce a register in which somebody appears to have agreed to something they were never asked about. An explicit approver always wins and is recorded as `explicit-approver`.

**Self-approval is recorded, never hidden and never refused.** Refusing would make the register useless to the solo developer this product is largely for; hiding it would let a formality look like a review. It is carried on the decision and stated in the mirror, on the page, and in the confirmation dialog before it happens.

**Requests transition; nothing is deleted.** `rejected`, `withdrawn` and `superseded` record three different things having happened. A supersession must name a request the register holds — the same refusal `defectRegister` makes about duplicates — and one whose successor has since gone returns to `pending` on read rather than staying closed by something nobody can find.

On the dashboard the Approvals page sits under *The work*, between Pull Requests and Director. Subjects are a **host-derived allowlist** built from outstanding roadmap items (by their durable anchor, never the positional id) and tracked documents; the webview posts an opaque option id and the host resolves it, so no message exists by which the register could be pointed at an arbitrary file. Currency is computed live against the subject as it stands now, which is what lets the page notice a rewritten document without writing to the register on a render. Recording a decision is the one register write on this dashboard behind a `{modal:true}` confirmation — a debt status is a note to yourself, this is a durable statement somebody later relies on — and it refuses when nobody is named, because an approval by nobody cannot answer the question the register exists for.

`buildApprovalReviewPrompt` may not decide: an agent that answered "approved" would turn the register into a machine agreeing with itself. It fences the rationale as REPORTED CONTENT (a request can be raised from an imported issue or a partner's email), states plainly that nothing it says about a legal-category request is legal advice, and asks for the thing people are worst at under time pressure — what would have to be true, what is not yet known, and what approving commits the project to that is hard to undo.

### DefectRegister (`src/core/defectRegister.ts`)

What is *broken*, as opposed to what was deferred. The dashboard already kept a register for what somebody found in the plan (gap analysis), in the code (tech debt), in the world (research) and in the business (risk), and none for the thing every project accumulates first and fastest. The Issues tab reads GitHub issues, which answers a genuinely different question: an issue is a public, filed artefact needing a repository, a remote and a working `gh`, while a bug is something somebody observed thirty seconds ago and will lose if there is nowhere to put it. Making a network round trip the price of writing one down is how the observation gets lost, so the register is a local committed file (`project_memory/operations/defects.json` plus a markdown mirror) and filing an issue stays a separate, confirmed act at the call site.

**Severity comes from a declared table and is never asked for.** A reporter asked *how bad is it?* answers about their own frustration; asked what it does — loses work, exposes something, does not work at all, works badly, looks wrong — and how many people meet it, they answer about the defect. `DEFECT_RULES` grades those two facts in eight ordered rules; `gradeDefect` is total by construction, with a test walking the product of both enums, because a table with a default is a table whose default eventually grades most of the register. Every entry names the rule that produced it and the table is published in the mirror.

**The grade is recomputed on every read.** `defects.json` is committed, so anybody can hand-edit a severity into it; a `"severity": "trivial"` on a data-loss defect does not survive `sanitizeDefectRegister`. Without that the rule table would describe the register rather than govern it.

**Data loss and a security exposure are blockers whatever their reach.** The two reach-independent rules come first deliberately: the one person it happened to lost exactly as much as if it had happened to everybody, and grading those on a headcount is the most defensible thing here to refuse.

**Reproducibility is its own field and does not move severity.** The classic mistake is to downgrade an intermittent bug, and it is exactly backwards — *sometimes* describes how confident we are that we can see it, not how bad it is when it happens. `not-reproduced` is a real state and is never quietly read as fixed.

**`fixed` is not `verified`.** A fix nobody checked is a claim, and the count that matters before a release is the verified one. `setDefectStatus` stamps `verifiedAt` on the transition rather than letting a date be inferred from whatever status an entry happens to hold today.

**A defect that came back is the same defect, reopened.** Recurrence lives on the entry as `reopenCount`, and it is derived from the transition — moving out of a closed status into an open one — rather than from a caller's flag, so a call site cannot forget it. Two rows would make a bug that has recurred four times look like four separate bugs each fixed once, which is precisely the shape that hides a chronic defect.

**Entries transition; nothing is deleted.** `verified`, `wont-fix`, `duplicate` and `not-reproducible` record four different decisions and only one is an accomplishment — the same argument `debtRegister` makes for keeping `resolved` apart from `obsolete`. A duplicate must name a target the register actually holds: `markDefectDuplicate` refuses an unresolvable one and the sanitizer drops a link whose target has left, because a dead cross-reference is indistinguishable from a live one to a reader.

Ids are minted from the title plus an ordinal, never from a clock or a random value, for the reason `roadmapGraphStore` mints them that way: the file is committed, and two developers recording the same defect on the same afternoon must not produce a diff that disagrees about its identity. Staleness is reported as its own fact (`staleDefects`) rather than escalating a grade, for the same reason severity does not drift with age in the debt register.

**Nothing here files anything and nothing here blocks anything.** An open blocker appears on the *Needs you* band as a statement; `releasePreparation` still owns release gates. In the attention feed the defects group is the one with **no `unassessed` rule**, deliberately: every other register can be *assessed* — a scan run, an advisor asked, a report read — so "never looked" is a state somebody can clear, while recording a defect means finding one. An item reading "no defects recorded" would nag a project that has genuinely found none, with no action that could satisfy it. The group is instead supplied only once something has been recorded, which keeps rule 5 intact from the other side: an unused register is not counted toward the groups that let the Overview claim `clear`.

**Handing a defect to an agent.** Unlike a debt entry, a defect report *is* untrusted third-party text — it can be pasted from a support ticket, an app-store review, or a colleague's message — so `buildDefectWorkPrompt` fences it as REPORTED CONTENT exactly as an issue body is fenced. The second half of the prompt is about what the agent may conclude: *cannot reproduce* is a first-class answer, because an agent that must find something will find something and a fabricated root cause recorded against a real defect is worse than an unresolved entry; and nothing in the prompt closes the record, since verification is a person checking, which is the whole distinction between `fixed` and `verified`.

**Agents are told to record what they find.** `buildDefectReportingGuidance` is appended to every role prompt beside `buildDebtMarkerGuidance`. The two are stated separately because confusing them is exactly what puts a real bug in the debt register and a deliberate trade-off in the defect one, and both exist for the same reason: an observation mentioned in a sentence of chat leaves no record, after which an empty register reads as an absence of the thing rather than an absence of recording.

The Defects page sits under *The code*, beside Tech Debt. The webview posts a defect's id and the facts it was told, never a severity and never a prompt — the grade is derived host-side and the agent's text is rebuilt from the register by id. `tests/views/defectsSurface.test.ts` reads both real sources and asserts those two properties, because breaking either would look like an ordinary refactor.
...
**A hand-off is passed through, structurally.** The chat panel answers some turns itself — a roadmap status summary, a conversation recall — without routing to a model, and `isRoadmapStatusPrompt` matched any prompt carrying both "roadmap" and a progress word. Every prompt built here ends with the sentence saying the model must not tick the item off, so all three carried "roadmap" and "complete" and all three were swallowed: AtlasMind answered its own instruction with a status dump, and the Completion check — the one hand-off whose whole job is to verify an item — was unreachable by construction. The bypass is a marker on the composer draft, not another pattern, on the rule already stated for `isCapacityDeferral`: a guard matching on wording is what failed, so wording cannot be what fixes it. Nothing but AtlasMind writes a composer draft, so a prompt sent unedited from one is AtlasMind's own text and never a question a deterministic responder may answer. The marker is one-shot and compared by exact text — edit the draft and what is sent is the operator's sentence, not ours — and a status question the operator typed is intercepted exactly as before. A property over all three builders holds it, so a fourth hand-off is covered without anyone remembering to add a case. A second layer narrows the matcher itself: an imperative opening a prompt or a line — `update`, `mark`, `resolve`, `draft` — is a request to act, never a question a summary may answer, so a typed "update the roadmap to mark the workflow item complete" is routed rather than answered with a summary of what had not changed. The two are not redundant, and the Completion check is the case that proves it: it opens with "Check", which no list of write verbs should contain since it asks for a report, so wording alone still swallows it and only the structural marker keeps it reachable.
...
Somebody else's roadmap, read into this one. The backlog could only be typed, which is fine for a project that started in AtlasMind and useless for every project that did not: the plan already exists, as markdown under `docs/`, as GitHub issues, on a Projects board, or in a spreadsheet somebody exported. The rules are all about not damaging a plan that already works.
...
**Checkbox state is provenance too.** `importedCompleted` records the source state alongside the title. A completion or reopen is an `update` only when the AtlasMind checkbox still equals that baseline; a local change or an older record with no checkbox baseline is a `conflict`, never permission to overwrite. A first import that finds an identical hand-written item returns `adopt`, which writes no backlog text and records the source link so the next rename or status change can be reconciled by key rather than rediscovered by wording.
...
Dashboard load runs the same planner as a bounded local drift gate, but not the same permissive parser as an explicit import. It searches only markdown whose filename contains `roadmap` or which lives under a `roadmap/` directory, excludes the complete configured AtlasMind SSOT, common SSOT backups, dependencies, Git internals, test fixtures, and known agent-worktree roots (including `.kilo`), caps files and bytes, and makes no network request. The host then resolves each surviving candidate and rejects it when any ancestor below the open workspace carries a `.git` file or directory, so an unknown tool's nested checkout is excluded by repository ownership rather than by hoping its vendor name is on a list. `parseAutomaticRoadmapItems` requires checkbox rows, refuses `# Plan: ...` documents and the Objective/Context/Approach/Steps/Verification scaffold, then skips rows under validation-only headings such as Acceptance criteria and Definition of done. Bullet-only narrative lists and those execution-evidence rows are not independently prioritised work. Repeated normalized titles across files produce one proposal; differing checkbox states refuse the title rather than letting lexical file order decide. The manual Markdown import stays broad because selecting a glob is an explicit intent signal. Adds, adoptions, renames, and checkbox changes are shown in a sectioned modal before `applyRoadmapImport` writes; conflicts and missing source items are only reported. The pass runs once per panel after durable anchors are ensured, so it cannot race a roadmap click or turn a render into a repeated writer. The modal says cancellation stops the listed import while separately naming anchor and managed-instruction maintenance that may already have run during Dashboard setup.

`assessAutomaticRoadmapItems` returns the rejected rows and the evidence that rejected them, but no delete instruction. **Check integrity** joins that evidence to `RoadmapImportRecord` provenance in the current graph, highlights only exact imported rows whose live source is a detailed plan or validation checklist, and leaves unreadable, missing-source, hand-written, and ambiguous entries unjudged. The webview sends no candidate ids. A native multi-select gathers the exact rows from the host-built findings, then a separate modal names `improvement-plan.md` and `roadmap-graph.md` before removal. Removing a selected row also removes only its node, touching edges, and dismissed suggestions; source documents are never edited.

**A duration means a different thing depending on who does the work.** The estimate table grades *scope*, and scope does not change with who picks an item up; elapsed time does, by enough that one scale cannot carry both. A person's figure is effort spread across working days; an agent's is wall clock in minutes. `RoadmapEstimateScale` splits them, chosen from the assignee: a Director contact whose `kind` is `agent`, or an id carrying `ROADMAP_AGENT_ASSIGNEE_PREFIX`. `MIN_ESTIMATE_DAYS` justified itself as *"a task somebody has to pick up and land costs a session"* — true of a person and of nobody else, and it forced every agent item to half a day.

`AGENT_MINUTES_PER_SCOPE_DAY` converts one scale to the other and is **a declared prior, not a measurement** — the rule text says so, because nothing here has watched anybody's agents work. One constant rather than a second table of bases: the scope judgement is already made and does not need making twice, and a reader who disagrees has one number to argue with instead of five. The **AI-assistance discount is not applied on the agent scale** — it grades a person working with AI help, and applying it to an agent counts the same fact twice; the toggle is withheld rather than shown disabled, since a control that changes nothing is worse than none. The roster is the source of truth so that changing the mark re-grades the plan, and an assignee resolving to nobody falls back to a person's scale — a real choice, not a neutral one, made visible by the node's existing unresolved-assignment chip.

**Route days accumulate to the minute, not the half-day.** Half-day rounding was exact while every estimate was a person's and became a defect the moment an item could take twenty minutes: three of them summed to zero, so a plan run by agents reported no work left. Human estimates are still multiples of half a day and are unaffected, which is asserted rather than assumed. `formatRoadmapDuration` renders a figure in the largest unit that does not round it away, because `0d of work left` is both wrong and the exact wording that makes somebody stop reading the column.

### CapabilityOffer (`src/core/capabilityOffer.ts`)

Offering a capability the project is visibly reaching for. A project that shells out to `gh` twenty times is telling you something; the risk in acting on it is that "we noticed you use X, install Y" is how a tool becomes a salesman, so every rule here exists to keep the offer rare and honest rather than to make it land.

Six rules. **Evidence-triggered, never speculative** — several *distinct runs*, not calls, because ten invocations inside one run is a project doing one thing once and counting calls would let a single afternoon manufacture a recommendation. **A signal AtlasMind already covers is not a gap**: `COVERED_BY_A_SKILL` excludes git, npm and docker, since using them is AtlasMind working and offering a server for one would recommend a second way to do a job that already works, through third-party code. **Never framed as a saving** — an MCP server publishes its whole tool list into the same tool budget a turn spends, and AtlasMind has observed that budget overflow and drop skills from a run, so `adds` and `consumes` are given equal weight and neither is optional. **A refusal is final**, per server per project, on any evidence however much stronger; an offer that returns when the count rises is a nag with a threshold. **One at a time**, because a list is a marketplace rather than an observation. **An offer is not trust** — nothing installs, and the setup path leaves the server seeded-disabled.

`SIGNAL_TO_SERVER` is deliberately short: every entry is a command whose only reason to appear is the service the server covers (`psql` is postgres and nothing else). A looser map — matching any word in a description — would produce offers from coincidence, which is the failure the whole module is shaped around. Refusals live in workspace state rather than `project_memory/`, because the SSOT folder is git-tracked and committing one person's "no" would put it in everybody's checkout as a diff nobody asked for.

The evidence itself is `ToolExecutionArtifact.commandName`, added for this: the **basename only, never the command line**, since an argument list carries paths, tokens, queries and file contents and none of that is needed to know which tool a project keeps reaching for — so the record cannot leak something the redactor would have had to catch. `commandSignal` takes a leading quoted token whole, because a Windows path with a space in it is the ordinary case and splitting on whitespace would yield the signal `program`, matching nothing and hiding a real one. Pure + unit-tested.

### CommitTrailers (`src/core/commitTrailers.ts`)

The machine-readable half of a commit message. A commit says what changed in prose; nothing said *which planned work it was for*, so every analytic wanting to join code to intent had to guess from wording — and a wrong join is worse than none, because it is counted rather than noticed. `roadmapCostAttribution` already makes this join for money; this makes it for commits, using git's own trailer convention so the answer is readable by `git log`, `git interpret-trailers`, and anything else this project ever points at its own history.

Five rules. **Only a link somebody already declared** — the issue number comes from the `<type>/<issue>-<slug>` branch shape the workflow file declares, never from the commit's prose, and a bare number elsewhere in a branch name is refused because taking it would permanently point a commit at somebody else's ticket. **A value is validated, never cleaned**, inverting the usual boundary rule here for the same underlying reason it normally applies: a pushed commit message cannot be edited, so a nearly-valid value made plausible is unfixable and the cost of being wrong is not symmetric. **Trailers follow git's own rules** — one block at the end, so `%(trailers)` sees them and an existing `Co-Authored-By` keeps its position rather than being displaced. **Composing is idempotent**: a key already present is replaced, so re-drafting does not accumulate copies. **Nothing here writes a commit** — it returns text a person still reads and still commits, the same gate `commitMessageDraft` relies on.

`splitTrailerBlock` implements the part of git's rule that matters: the last paragraph counts as trailers only when **every** line in it is `Key: value`. Taking the lines that happen to match would split a sentence out of somebody's closing paragraph and call it metadata. That agreement was verified against real git rather than inferred — a commit whose final paragraph reads `See also: the notes` followed by a sentence is read as prose by both `%(trailers)` and this parser.

The dashboard's commit log reads them through git's own `%(trailers)` rather than parsing bodies, with ASCII group/unit separators in the format because a trailer block is multi-line by definition and splitting records on newlines would cut every linked commit into pieces. Values are re-validated on the way back in: a hand-edited history can contain anything, and a surface joining on a malformed id would report a link to an item that does not exist. Pure + unit-tested.

### AgentCapacity (`src/core/agentCapacity.ts`)

Whether the team you configured can work, and whether it is used. The dashboard counted agents, skills and healthy providers and did nothing with the answer: `4/9 providers healthy` sat in a stat card's subtitle, in the same grey as everything else, on a page whose job is to say what needs a person. A project can be perfectly configured and unable to route a single request, and nothing said so.

Five rules. **A provider that is down is news, not a grade** — it reaches the attention feed and never the score, because an outage this morning is not a fact about how well a project is run and a score that moved with one would be a weather report people learn to explain away (asserted by a test that the score component is identical for a healthy and a wholly broken estate). **Disabled is a decision, not a gap**: only enabled agents can be idle, or a tidy configuration reads as a problem. **Unassessed is not idle** — no run history means nothing was observed, so `utilisation` is `undefined` rather than `0`, the component is dropped entirely, and the derived denominator keeps the headline honest without anybody adjusting a total. **The join is by role, not agent id**, because a planner subtask runs as an ephemeral agent carrying a role and no registry id; matching on id would report a constantly-busy project's whole team as idle. **Nothing routable is a harder stop than an unhealthy provider** — a provider with no enabled model cannot be reached at all, and equal weighting buries the difference between degraded and unable to work.

`capacity-blocked` is the **first** rule in `attentionFeed`, above a red pipeline: a failing test is a problem you can work on, and no routable model is the one condition under which nothing else on the page can be acted on. `capacity-degraded` is `soon` and names which providers failed. A finding about idle agents is withheld below `MIN_RUNS_TO_JUDGE_UTILISATION`; the figure is still computed, because it is a true statement about what has been seen, but three runs is not evidence that six agents are surplus and saying so would have somebody switch off a team they are about to need. The reading is carried on the snapshot rather than recomputed per surface — the Overview raises it, the Runtime page explains it, and the score reads its utilisation, and three readings of one fact would eventually disagree. Pure + unit-tested.

### RoadmapCriticalPath (`src/core/roadmapCriticalPath.ts`)

Which chain of work decides when the plan lands, and what has room to slip. The roadmap answered two questions and not this one: an ordered list says *this matters more than that*, and the dependency graph says *this cannot start until that lands*. Neither says which chain the finish date rests on — so a backlog can be correctly prioritised, correctly sequenced, and still have everybody working on the items that were never the constraint.

Five rules. **Only outstanding work is on the path** — completed prerequisites are drawn because they explain how you got here, so counting their days would make a history out of a forecast. **The finish is the longest chain, never the sum**: independent work runs at the same time, and adding estimates up is the mistake this exists to stop somebody making by hand, always erring pessimistic, which is the direction people stop believing. **Slack is measured against the plan's own finish, never a deadline** — `describeRoadmapSchedule` already grades a node against its deadline and says so, and folding one in here would make a number meaning two things with no surface able to say which. **A plan with a cycle has no finish date**, reported as circular rather than given a number, the same call `resolveRoadmapGraph` makes when it names a cycle instead of breaking it. **Nothing outstanding is not a zero-day plan** — an empty backlog and a backlog whose work takes no time are different statements.

The forward pass is **not recomputed**: every node already carries `schedule.routeDays`, the longest run of outstanding work into it, and a second implementation of the same walk would eventually disagree with the number printed on the card beside it. The backward pass walks dependents in reverse depth order, which a layered DAG guarantees is valid, and skips delivered dependents — one that is already done is not waiting on anything. Arithmetic is in **half-days as integers**, not fussiness: the whole result turns on `slack === 0`, and comparing accumulated floating-point sums is exactly what would put an item on the path on one machine and not another. Negative slack is clamped, since it can only come from a graph disagreeing with itself and would send somebody hunting a scheduling subtlety that is really a data fault. Pure — no clock, no `fs`, no model — and unit-tested.

On the canvas it is a **third emphasis lens** beside gate and person, combining with them rather than replacing them, and it dims rather than hides: the items with slack are the comparison that makes the answer worth having. Absent on the Delivered record, where the path means nothing and a lens matching nothing reads as broken rather than inapplicable. The finding is stated whether or not the lens is on, because this is the one lens that answers a question rather than narrowing to an answer you already had. Shipped with the snapshot like every other view state here, so looking at it cannot fail.

### RoadmapTimeline (`src/core/roadmapTimeline.ts`)

The plan against time — when each item can start, when it can finish, how much room it has before the finish moves, and where each gate lands. The canvas shows *order* and the backlog shows *priority*; neither shows **duration**, so nothing said that four items sit idle for a week waiting on one, or that a gate lands after the deadline it is tagged for. A list cannot show simultaneity and a graph cannot show length.

Seven rules, and the first two decide what the chart is allowed to claim. **One schedule, not two** — every bar is positioned from the earliest and latest finishes `roadmapCriticalPath` already computed, because a second forward pass here would eventually disagree with the number printed on the node's own card, and a Gantt that contradicts the card beside it is worse than no Gantt. **A bar is a duration, never a date**: the axis is days from today and the only real dates on it are deadlines somebody declared, because `roadmapGraph` grades a human item in effort spread across *working* days and an agent item in *wall clock* — turning a mixed chain into calendar dates would mean inventing a working calendar, a holiday list and a working day's length that nobody declared, and printing "12 March" from that would be a commitment made up by a renderer.

**A deadline grades, it never schedules** — bars are placed by dependencies and estimates alone, since a deadline that pulled its bar earlier would draw a plan that always meets it. **Float is measured against the plan's finish, not the deadline**, inherited from `slack-against-the-plan` and restated because on a chart the lighter tail *looks* like spare time before something external. **A milestone is dated only when every member is**: a gate with an unschedulable item gets no date rather than the maximum of what is known, which is indistinguishable from a complete maximum once drawn and always reads early. **Delivered work is counted, never drawn** — it takes no time, so a bar for it would be a history lesson occupying the part of the chart that forecasts. **A plan with a cycle has no timeline**, for the same reason it has no finish.

Pure — no clock, no `fs`, no model; day zero is "now" by construction because that is what the critical path's numbers are relative to. Gate labels are passed in by the caller rather than looked up, since only the panel knows what this project called `#mvp` and a module inventing a label would print a different name from the selector two rows above it. Ordering is start, then how little room there is, then id, so two items that begin together cannot swap places between renders. Unit-tested.
...
Six rules. **Severity comes from the publisher, never re-graded here** — GitHub's advisory database and CodeQL both ship one, and a local score would produce a number nobody else's tooling agrees with; an advisory with no severity stays `unknown` rather than defaulting to something reassuring, and a code-scanning rule's `warning`/`error` lint grade is never mistaken for a risk grade. **A dismissal is a decision, not a fix**: dismissed alerts are counted apart and never folded into anything reading as resolved, or a project can dismiss its way to a clean board — while `fixed` and `auto_dismissed` are left out entirely, being history rather than work. **Unassessed is not clean** — a source nobody read says so, and zero open is claimed only where something looked. **A feature that is switched off is a finding, not an absence**: a repository with Dependabot alerts disabled is reported as disabled, since showing it as an empty list would make the riskiest configuration look like the safest. **The text is third-party** — control-stripped, clamped, capped, non-`https` links dropped, and nothing throws. **Ranked by consequence, capped, remainder stated**, interleaving both sources by severity rather than grouping by source, because grouping buries a critical dependency alert under a page of medium code findings.

Pure — no `fs`, no network, no clock. The panel runs `gh` on the **repository refresh** and never on render (these are rate-limited calls and the Security page re-renders on every keystroke elsewhere), and classifies each half: a 403 or 404 from these endpoints is GitHub saying the feature is off, which is a fact about the repository rather than a fault in the read. The card's one outbound control sends `<source>:<reference>` and never a URL — the host resolves it against the advisories it actually read, the same rule `githubDeepLinks` follows, because a surface that could name a URL could name any.
...
The outbound counterpart to `aiInstructionSync.ts`. `syncTestingProtocols(workspaceRoot, config, agents)` renders the enabled methodologies into a delimited, AtlasMind-managed markdown block (`<!-- atlasmind:testing-protocols:start -->` … `:end -->`) and upserts it into every *detected* (existing) external agent instruction file — `CLAUDE.md`, `.github/copilot-instructions.md`, `AGENTS.md`, Cursor, Cline, Gemini, Windsurf, Aider. The same pass owns independent debt-marker, workflow, and roadmap-SSOT blocks. `syncRoadmapInstructions` can install the roadmap block without a testing configuration; Roadmap dashboard load calls it and seeds `AGENTS.md` when absent, while preserving every byte outside the managed block. The rule names the configured `project_memory/roadmap/improvement-plan.md` path, requires any secondary roadmap edit to be reconciled there in the same change, and tells the agent to report rather than guess on conflict. Every path passes the shared `isSafeRelativePath` / `resolveRelativePath` traversal guard. JSON-config tools are reported as skipped.
...
- `gatherInstructionSources(workspaceRoot)` reads the full authored content of every detected tool file plus AtlasMind's own canonical instructions (`project_memory/agents/atlas-personality-profile.md`, `project_soul.md`), stripping AtlasMind-managed shared, testing, and roadmap blocks so the merge never re-ingests its own mirror.
...
- `runInstructionRender` / `renderUnifiedMarkdown` re-express the unified set in each tool's native format (deterministic fallback when the model omits a tool); `applyManagedInstructionBlock` upserts the result into each detected file's `<!-- atlasmind:shared-instructions:start -->` block and refreshes the separate roadmap block on every seeded target; `writeUnifiedToSsot` mirrors the set to `project_memory/domain/ai-instructions-sync.md`.
...
- **`ArdInstaller` (`src/ard/ardInstaller.ts`)** — maps a discovered resource to a non-destructive action: MCP servers → `McpServerRegistry.addServer({ enabled: false })`; nested catalogs/registries → disabled finders; A2A/skill/API → reference only. `atlasmind.ard.installEntry` now asks first: a `{modal:true}` confirmation whose detail is `describeInstallPlan(resource)` — the finder, the source URL, that relevance is not trust, and for an MCP server the exact stdio command line (or remote URL) and the env var names, stating it is added switched off. It previously installed with no confirmation, which mattered less while the only way to reach it was a person searching by hand.
...
- **Chat fall-through (`src/core/capabilitySearch.ts`)** — what chat does when it needs a tool nobody installed. When the model's `find-tool` search finds no installed skill, the Orchestrator asks the injected `ExternalCapabilitySearch` (`setExternalCapabilitySearch`). `activate()` wires it to `ardRegistry.listEnabled()` and `ardClient.searchEndpoints(..., { maxResults: 5 })`, re-reading `atlasmind.ard.enabled` on every call, and merges results into the registry's recent-results cache (capped at 50) so an install button can resolve them later. Four rules. **Enabling a finder is the consent** — finders ship disabled, and with none enabled the model is told so and told to point the user at Settings → Resource Discovery, because silence would read as "no such tool exists", which nobody checked. **Only the query leaves the machine** — `prepareCapabilityQuery` passes the `find-tool` text through `redactSecrets` and clamps it to `MAX_CHAT_DISCOVERY_QUERY_CHARS` (160); never file contents, never the conversation. **Discovery installs and grants nothing.** **Relevance is not a trust rating**, and every result says so. Up to `MAX_CHAT_DISCOVERY_RESULTS` (5) candidates reach the model through `describeCapabilityOutcome` (which also composes the no-finder and error cases, so a caller handling only success cannot drop them) and the reply through `TaskResult.discoveredResources`. `shouldOfferToolDiscovery` (`toolDiscovery.ts`) takes a third argument, `externalSearchAvailable`, so `find-tool` is offered even when every installed skill was already sent — but still never on a turn given zero tools.
- **Install from chat** — the VS Code chat participant renders one **Review & install: <name>** button per candidate (`stream.button` → `atlasmind.ard.installEntry`). The AtlasMind chat panel renders the same buttons from `SessionTranscriptMetadata.discoveredResources` (`SessionDiscoveredResource` in `src/chat/sessionConversation.ts`: `identifier`, `displayName`, `type`, `sourceName`, `score?`) and posts `{ type: 'installDiscoveredResource', payload: { identifier } }`. `chatProtocol.ts` accepts a non-empty string of at most 512 characters and nothing else, and the command resolves it against the host's own recent results — a crafted message can name a resource but never describe one. Either path ends at the confirmation above.
...
| `TaskResult` | Agent ID, model used, response, cost, duration; `modelAttempts`; optional `discoveredResources` (`ArdDiscoveredResource[]` — Resource Discovery candidates found after a `find-tool` miss, rendered as install buttons) |
...
**A pill can move the checkout, and the guards are what make that acceptable.** A pill naming a branch you are not on is a button; the stage you are standing on carries a coloured outline. The webview posts the pill's **id**, never a ref, resolved against the strip the panel last sent — so a message can name a stage that exists and can never introduce a branch name of its own. The working-tree pill is refused, having no branch by design. The branch must already exist locally, because a click on a version number must not create a stage branch as a side effect. Uncommitted work is counted and named in the confirmation rather than discovered afterwards, and nothing on the path forces, stashes, resets or discards. `git checkout <branch> --` is used rather than `git checkout -- <branch>`: with the disambiguator first the name is read as a pathspec and the command silently becomes "restore the file called main from the index", discarding uncommitted work on that path instead of switching branch. A test pins the argument order, and matches quoted git arguments rather than bare words — the method's own comment contains the word "stashes" precisely because the code does not.
