...
Communication between webview and extension uses `vscode.postMessage()` / `onDidReceiveMessage()`. Treat all incoming messages as untrusted and validate them before changing state or touching secrets.
 ...
 The shared Atlas chat webview now also hosts live tool-approval cards, so approval-response messages must be validated with the same strict message guards as prompt submission, voting, attachment flows, and the composer history shortcuts that recall recent submitted prompts from persisted webview state. Those approval cards sit below the transcript and just above the composer, use warning-accent styling rather than blending into the message list, and should reuse the currently active chat surface instead of spawning a second detached panel when attention is needed. The composer accepts common Enter variants as well, but they now map to explicit chat actions: Enter uses the currently selected send mode, Shift+Enter starts a new chat thread, Ctrl/Cmd+Enter sends as Steer, and Alt+Enter inserts a newline. When the panel is on a chat surface instead of a run inspector and AtlasMind is idle, state refreshes restore focus to the composer by default so consecutive prompts do not require another click.
...
The Settings panel (`src/views/settingsPanel.ts`) now includes validated controls for `/project` execution behavior in addition to budget/speed modes, provider shortcuts, and text-to-speech tuning for AtlasMind voice playback. Numeric fields are constrained to positive integers, ranged numeric voice inputs stay within safe bounds before persisting, and report-folder input is required to be non-empty before persisting. Its hero banner also keeps the installed AtlasMind extension version visible in the lower-right corner so operators can confirm the loaded build without crowding the main title.

The Personality Profile panel (`src/views/personalityProfilePanel.ts`) is a guided questionnaire webview that combines editable role, tone, memory, and boundary prompts with live AtlasMind configuration values such as budget mode, speed mode, approval mode, and chat carry-forward limits. Each prompt now keeps a freeform text area as the source of truth while also exposing quick-fill presets so operators can seed a response without losing the ability to write custom guidance. The panel can save the current form either as a global default or as a project-specific override. Global saves persist the questionnaire in VS Code global state and write the live AtlasMind settings to user scope, while project saves keep using workspace state plus workspace settings. A dedicated revert action clears the project override and restores the saved global baseline for both the questionnaire and the live settings. When SSOT is available, only project-specific saves mirror into `project_memory/agents/` plus a synced summary block in `project_soul.md`, which prevents user-wide defaults from leaking into repo-managed memory. The extension runtime now merges the saved global profile with any project override, then combines that effective operator profile with a compact summary of `project_soul.md` so both the operator preferences and the project identity influence every request instead of staying passive documentation.
