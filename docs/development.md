...
The Project Dashboard (`src/views/projectDashboardPanel.ts`) is the high-level operational overview surface. It composes local git state, Atlas runtime status, Project Run History, SSOT coverage, memory scan warnings, workflow inventory, dependency-governance scaffolding, and aggregate `/project` TDD posture into one interactive panel with animated chart cards and adjustable 7-day, 30-day, and 90-day timelines. It now also hosts a guided ideation workspace: a collaborative whiteboard page with draggable cards, card linking, focused-card inspection, Atlas feedback history, queued follow-up prompts, browser-side voice capture and narration controls, and image attachments for multimodal ideation passes. The board persists to `project_memory/ideas/atlas-ideation-board.json` plus a markdown summary so ideation sessions remain reviewable outside the live panel. The runtime section still highlights blocked or missing failing-test evidence alongside per-run TDD labels so operators can spot autonomous-delivery issues before drilling into the Project Run Center.
