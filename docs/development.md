...
The shared Atlas chat webview now also hosts live tool-approval cards, so approval-response messages must be validated with the same strict message guards as prompt submission, voting, attachment flows, autonomous-run review actions, and the composer history shortcuts that recall recent submitted prompts from persisted webview state. Those approval cards sit below the transcript and just above the composer, use warning-accent styling rather than blending into the message list, and should reuse the currently active chat surface instead of spawning a second detached panel when attention is needed.

Autonomous runs launched from chat now remain anchored to that same surface. The Sessions rail renders chat-linked runs as nested child rows under their parent thread, assistant footers can reopen the matching inline review bubble, and the composer shows a flyout bar whenever linked run files are still pending accept-dismiss review. Treat those review messages with the same validation standard as any other webview command because they mutate persisted run metadata.
