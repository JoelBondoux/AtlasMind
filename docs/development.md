...
All Project Dashboard chat handoffs pass through `openDashboardChat()` and the pure `planDashboardChatDispatch()` boundary. The per-workspace `atlasmind.dashboard.chatDestination` setting defaults to AtlasMind, where the target is forced to `autoSubmit: true`. External choices are enumerated from installed `chatParticipants` and `chatSessions` manifest contributions, never inferred from arbitrary commands, then re-resolved at click time. Treat contribution ids and labels as untrusted: bound labels, allow only the closed token grammar, construct only the two fixed VS Code chat command shapes, and refuse a missing saved destination rather than silently falling back. External routes receive only the prompt; AtlasMind-only `contextPatch` and `directResponse` fields must never be serialized into them.
