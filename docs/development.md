...
Actionable failures and explanatory cards should use the shared `ATLAS_DISCUSS_ACTION_CSS` / `renderAtlasDiscussAction()` affordance rather than introducing panel-specific “Ask Atlas” chrome. A roomy error row shows the AtlasMind logo plus **Resolve with Atlas**; dense informational cards may use the compact logo as long as `aria-label` and `title` name the topic. Prefer an opaque record id in the webview message and re-read the current record in the extension host. Redact, bound, and fence any process-, tool-, or repository-derived text as reported data before putting it into an unsent Chat draft. The MCP error and Project Dashboard Testing Policy Coverage flows are the reference implementations.
...
The Project Dashboard also includes an **Issues** page (backed by `src/core/issueTracker.ts` and the `gh` CLI) showing the repository's tracker: open / unassigned / stale counts, open issues by label and by assignee, a search-and-filter list, and per-issue **Work on it with Atlas**, **Comment**, **Close/Reopen**, and **Open on GitHub**. Reads are user-triggered (`refreshIssues`) rather than part of a dashboard render, because `gh` is a rate-limited network call; the fetched list lives on the panel and is carried through renders. Writes are confirmed modally with the repository and exact action named. The nav badge only appears once the tracker has actually been read, so an unloaded list can never read as a quiet tracker. The Project Dashboard also includes a **Risk** page (backed by `src/core/riskOversightManager.ts` and `project_memory/operations/risk-oversight.json` + a `risk-oversight.md` mirror and `risk-oversight-history.json` audit trail) that runs the three read-only oversight advisors, records what they find, scores it into the operational health number, and charts it. Runs are explicit and user-triggered — per-domain or all three **sequentially**, never concurrently, since three parallel model calls is a surprising cost from one click — with live per-advisor progress via `riskBusy`/`riskStatus` messages. The advisor is *pinned* with `orchestrator.processTaskWithAgent` rather than routed, so the page always consults the advisor requested. Because the advisors are read-only, the panel owns the write path: it parses the model's JSON defensively (`parseRiskFindings` never throws), sanitises it (`sanitizeRiskFindings`, path-traversal rejected in cited evidence), and merges it without undoing human decisions. Charts are hand-rolled under the existing CSP (no external library): a likelihood × impact **risk matrix** heatmap in CSS grid whose cells filter the register, plus the shared `renderChartCard`/`renderScoreRing`/`renderMetricPill` primitives. Risk is **excluded from the score until a project has actually been assessed**, so an unassessed project reads as unknown rather than safe.
