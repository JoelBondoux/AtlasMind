...
**Debugging from source (F5) needs no packaged build.** The launch config points at the workspace folder and its finite `build-extension` pre-launch task runs `npm run compile`, then exits before the Extension Development Host starts. It deliberately does not retain `tsc -watch`: a long-lived watcher consumes substantial committed memory and can make a new Node extension host abort before its logger exists on a busy workstation. Both profiles also set `experimentalNetworking` to `off`; VS Code 1.137.0's debugger otherwise starts the host with Node's experimental network inspector, which emitted repeated `Missing dataLength in event` exceptions during a successful AtlasMind activation. The default **Run Extension** profile additionally disables built-in GitHub Copilot Chat because the same VS Code/Copilot versions have an open exit-code-134 regression ([microsoft/vscode#335916](https://github.com/microsoft/vscode/issues/335916)). Use **Run Extension (Copilot integration)** only when the test specifically needs the VS Code Copilot provider.
...
Press **F5** in VS Code to launch the Extension Development Host. The extension activates on startup (`onStartupFinished`). Both profiles finish a full compile before launch and turn off experimental Node network inspection while retaining ordinary breakpoints, stepping and source maps. The default **Run Extension** profile is the stable source-debug path and explicitly disables built-in GitHub Copilot Chat in addition to other installed extensions. Select **Run Extension (Copilot integration)** from the Run and Debug menu only for Copilot-provider integration work. If a host still exits with code 134 before the AtlasMind output channel appears, check Windows committed-memory headroom: that point precedes AtlasMind activation and is a native extension-host abort, not a roadmap-load exception.
...
When a dashboard action delegates work to the Orchestrator, make the lifecycle visible in the panel as well as in VS Code notifications. The activated-testing repair flow is the reference: the extension host sends a started event, concise real routing and approved-tool updates, and one terminal completed or failed result. The webview uses an indeterminate progress control while work is active rather than inventing a percentage, preserves the reported output for review, and distinguishes “task completed” from “tests are green.” Any Chat handoff is host-owned: the browser sends only the bounded action/id, and the extension host rebuilds, redacts, bounds, and dispatches the prompt. A prompt-bearing Atlas action is itself the submit gesture; do not regress it to an unsent composer draft.

All Project Dashboard chat handoffs pass through `openDashboardChat()` and the pure `planDashboardChatDispatch()` boundary. The per-workspace `atlasmind.dashboard.chatDestination` setting defaults to AtlasMind, where the target is forced to `autoSubmit: true`. Settings and Dashboard both resolve it through `resolveDashboardChatDestinationSetting()`: explicit registered configuration wins, then a validated workspace-state fallback covers an Extension Development Host whose outer window has not registered a newly contributed setting, then the manifest default applies. A successful settings write clears the fallback. External choices are enumerated from installed `chatParticipants` and `chatSessions` manifest contributions, never inferred from arbitrary commands, then re-resolved at click time. `collectDashboardChatDestinations()` exposes one primary participant per extension because editor/notebook/agent/terminal participant IDs are internal routes rather than separate services; their validated IDs remain aliases so saved selections migrate to the primary route. Distinct declared chat sessions remain separate. Treat contribution ids and labels as untrusted: bound labels, allow only the closed token grammar, construct only the two fixed VS Code chat command shapes, and refuse a missing saved destination rather than silently falling back. External routes receive only the prompt; AtlasMind-only `contextPatch` and `directResponse` fields must never be serialized into them. Settings renders the shared `renderAtlasDiscussAction()` button as both the visual example and the live test control. It posts only the selected destination id; the host re-resolves that id from installed contributions and uses `planDashboardChatDispatch()` to send a fixed test prompt, so no prompt or command supplied by the webview crosses that boundary.
...
`websiteStudioPanel.ts` and command id. Its shell is a **Surfaces rail** beside a set of unnumbered
...
Worktree isolation runs git through `OrchestratorHooks.runGit`, not through `context.runCommand`, and the split is deliberate. It needs stdin, which `git apply -` requires and no skill has ever wanted, so routing it through the skill context would widen the surface every model-driven tool sees for the sake of one caller that is not a tool. Every command it issues is a module-level constant in `src/core/worktreeManager.ts` or `src/core/worktreeMerge.ts`; nothing is composed from a model's output, and nothing on that path goes through tool approval. If you add a command there, keep it a constant with an argument array, and never take a fragment from a caller.

Two details of the hook are load-bearing rather than incidental. It declares `encoding: 'buffer'`: without it Node's child streams emit strings, `Buffer.concat` throws on the very first call, and the failure is indistinguishable from a repository that cannot make worktrees — the feature would look switched off. It also means a patch is decoded once at the end rather than per chunk, so a multi-byte character split across a chunk boundary survives. And a non-zero exit rejects with **stderr**, because git's explanation of why a patch would not apply lives there and that sentence is what reaches the operator.

The four `worktree*` modules under `src/core/` are pure or dependency-injected and unit-tested, but the git behaviour itself was verified by execution against a probe repository rather than inferred from documentation. If you change what is sent to git, re-verify the same way: that a newly created file survives the round trip (it needs `git add --intent-to-add` before the diff, or it is silently dropped), that a conflicting patch is refused with no markers written into the working tree, and that the parent repository's status afterwards shows only the merged changes.

**A `ChatPanel` can outlive its surface, so do not assume `this.host` is live.** When a chat is closed or its view is discarded, `dispose()` swaps `host` for the inert one in `src/views/chatBackgroundRuns.ts` and lets the run finish. That is why `host` is not `readonly` and why the fix was not a guard on each `postMessage`: there are over a hundred call sites, and one missed guard throws "Webview is disposed" into the middle of a run and ends it. If you add a path that reaches the webview, post through `this.host.webview` rather than capturing it into a local or a closure that outlives the call — a captured live webview survives the swap and reintroduces exactly that throw. Anything that must still happen for a detached run belongs on `sessionConversation`, which is the transcript's real home and is written before the webview on every chunk.

**A chat-panel button names a thing; it never describes one.** The "Review & install" buttons under a reply are rendered from `SessionTranscriptMetadata.discoveredResources` and post `{ type: 'installDiscoveredResource', payload: { identifier } }`. `isChatPanelMessage` in `src/views/chatProtocol.ts` accepts a non-empty identifier of at most 512 characters and nothing else, and the panel forwards it to `atlasmind.ard.installEntry`, which resolves it against the host's own recent Resource Discovery results and shows its own confirmation. Follow the same shape for any new webview action that ends in an install or a write: the browser supplies an id, the host supplies everything that id means. `tests/views/webviewMessages.test.ts` pins the validator.
...
`publish:release` runs `vsce publish` and nothing else, authenticating with whatever credential `vsce login` stored in the OS keychain — it is the emergency path for publishing from a developer machine. **CI uses `publish:release:ci` instead** (`vsce publish --azure-credential`), which authenticates as the managed identity `vscode-marketplace-publisher` through workload identity federation; there is no Marketplace secret in the repository. The two are kept separate because adding `--azure-credential` to the local script would break publishing from a machine that has no Azure sign-in. Tagging is `npm run tag:release`, which creates and pushes a `v<version>` annotated git tag (`.github/scripts/tag-release.mjs`, cross-platform and idempotent — it skips if the tag already exists). The two are deliberately **not** chained: the tag push triggers `publish.yml`, so chaining them made one release attempt two publishes, the second failing on "version already exists". Normal flow is `tag:release` locally, then CI publishes from the tag. Since `@vscode/vsce` 4.0, `vsce` needs Node.js 22 or later (CI runs 24), and the first local `vsce publish` after upgrading may offer to migrate a legacy PAT stored by `vsce login` — expect that prompt on the emergency path, not in CI, which never uses a PAT.

The checked-in `.gitignore` keeps the local `project_memory_old/` backup outside source control, and `.vscodeignore` is the packaging boundary for local and release VSIX files. It intentionally excludes workspace-only content such as all `project_memory*` directories (including local archive or backup variants), local `.kilo` worktrees, a top-level local/generated `website/` tree, `wiki/`, local `.vsix` outputs, Vitest JSON report artifacts, Stryker's `.stryker-tmp/` sandbox, separate test/e2e/performance trees, assistant instruction folders, and extra dependency test or docs folders so the packaged extension stays closer to runtime-only contents. `tests/packageManifest.test.ts` pins the generated directory exclusions. Review the `vsce package` file listing before publishing; workspace memory, an assistant worktree, or a project website in that listing is a release blocker. Never bypass `vsce`'s secret scanner to admit one of those directories.
...
## Roadmap reconciliation integrity

Roadmap reconciliation has two Markdown parser policies. Explicit **Import...** remains broad because
the user chose its source; the automatic load-time pass rejects detailed plan scaffolds and
validation-only checklists before it proposes work. **Check integrity** joins current source
