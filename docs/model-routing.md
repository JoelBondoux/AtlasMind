...
- **ACP** encloses a process spawn, `initialize`, `session/new` and then the prompt itself, so its deadline is the adapter's per-request budget **plus handshake headroom** (`ACP_REQUEST_TIMEOUT_MS + ACP_HANDSHAKE_HEADROOM_MS`), derived from the inner budget rather than restated. When the two numbers were independently both 180 seconds, the enclosing timer always fired first on a cold start: a slow handshake surfaced as a bare "Provider timed out after 180000ms" naming no phase, and the adapter's own message — which names the method that stalled — was never the one the user saw.
