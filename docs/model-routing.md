...
If the selected provider fails outright, AtlasMind attempts **up to three failovers** for the turn, under a hard backstop of five model attempts in total. Failover and escalation draw on **separate budgets**: escalation is the discretionary upgrade for an answer that was merely not good enough, failover is what keeps a turn alive when an endpoint dies, and a single shared counter used to let the first spend what the second would need.

When a turn does stop, the report **leads with what failed** — every attempted model, what happened to it, and how long it took — and names the exhausted limit afterwards. The budget explains why nothing else was tried; it is not why anything failed. A diagnosis is offered only when the attempts agree: an all-timeout turn is stated as an endpoint problem rather than a model one, because no model reported a fault. Mixed failures get the list and nothing more, since inventing a common cause across unrelated failures sends the reader to one fix when several are needed — the exact defect this replaced, which quoted the last provider's error and reported a turn that had lost three endpoints to three different causes as one provider problem.

**Non-conversational models are never routing candidates.** A provider's model list is an inventory of what it serves: a local runtime enumerates every set of weights it has loaded, and OpenAI's list carries `text-embedding-3-large`, `whisper-1` and `dall-e-3`. `src/providers/modelRole.ts` classifies these against a short published rule table matched on **name segments** (never bare substrings), and an unrecognised model is always treated as conversational — a false positive would silently hide capacity the user installed. Discovery drops them, capability inference withholds every capability from them, and `ModelRouter` refuses any model that does not declare `chat`. That last gate is the enforcement rather than the documentation: `requiredCapabilities` never names `chat` because it is assumed, so a free local guard model with a full inferred capability set was an ordinary candidate — and won the last failover of a turn before answering with a chat-template error.

**Conversational does not imply transport-compatible.** `OpenAiCompatibleProviderConfig.modelIdFilter` is the provider-specific gate for models that can converse only through another protocol. Google lists Gemini Live models beside ordinary generation models, but those models require the stateful bidirectional Live API rather than the stateless chat-completions adapter. `isGoogleChatCompletionsModel` therefore withholds model ids carrying the declared `live` segment before they reach the router; ordinary Gemini text models remain eligible.
...
Account-scoped refusals are separate from endpoint faults. A 429 skips the provider for the rest of the turn without marking one model bad. Billing failures pause it for the session. A 401, or a narrow 403 that explicitly names an API key, account, project, or `PERMISSION_DENIED`, now pauses it as an authentication/access failure and fails over after one attempt. A bare model-specific 403 is deliberately not widened to the whole provider.
...
**Failover never replays a side effect.** An attempt is the whole agentic loop, tools included, so failing over re-runs the task from the top on another model — harmless for reads, wrong for a commit or a push, and observed as "commit, push and promote" being handed to model after model. Each attempt records the tool calls it *started* that `isSideEffectingToolCall` classifies as able to change something (only the `read`, `git-read`, `network-read` and `terminal-read` categories are safe; unknown counts as side-effecting). If the attempt then fails, the turn stops with a reply naming what had started and advising a look at `git status` / `git log -3`, rather than failing over. `describeReplayHazard` applies the same stop to a delegated-tool ACP attempt that failed once its prompt was in flight, since the agent's own tools may have run and AtlasMind cannot see which; a failure during spawn, handshake or sign-in still fails over. Escalation is skipped on the same condition.

An ACP agent that stops answering mid-prompt ("did not answer", "went silent", "was still working") has its endpoint scope blocked for the rest of the turn: every model behind that agent is the same process, and the observed failure was four Codex models in a row, each waiting out the full timeout.

**Deadlines are sized to what the attempt has to do.** The generic 30-second provider deadline is written for a hosted HTTP call, where the weights are resident and somebody else owns the GPU. Two provider shapes need more:

- **ACP** encloses a process spawn, `initialize`, `session/new` and then the prompt itself, so its deadline is the adapter's prompt ceiling **plus handshake headroom** (`ACP_PROMPT_CEILING_MS + ACP_HANDSHAKE_HEADROOM_MS`), derived from the inner budget rather than restated. Inside it, `session/prompt` runs on an **inactivity** timeout (`ACP_REQUEST_TIMEOUT_MS`, 180 s) that every `session/update` or agent request restarts, bounded by `ACP_PROMPT_CEILING_MS` (30 minutes): a fixed 180 s declared a Codex agent hung while it ran a commit hook, the tests and a push, streaming progress the whole time. When a prompt times out the adapter sends `session/cancel`, so the agent does not carry on in the background. When the two numbers were independently both 180 seconds, the enclosing timer always fired first on a cold start: a slow handshake surfaced as a bare "Provider timed out after 180000ms" naming no phase, and the adapter's own message — which names the method that stalled — was never the one the user saw.
- **Local** does the model loading and the prompt evaluation on this machine, so the budget scales with the parameter count read from the model id, the measured prompt size, and whether the weights are already resident (the first attempt against a model in a session pays a cold-start allowance; later ones do not). Unknown inputs widen the budget or leave it alone and never narrow it — a prompt nobody measured must not shrink a deadline. The result is clamped to five minutes. A 14B model on a five-thousand-token cold prompt was previously recorded as a timeout at 30 seconds, which quarantined a working endpoint and spent a failover.

The same size hint crosses the local adapter's GPU-admission boundary. Without it, the arbiter correctly used its conservative unknown-model assumption, but did so for every routed model; an explicit `qwen3-8b` was therefore priced as an unknown 16 GiB load and could wait 45 seconds on a card with enough room. A model id that states no size still takes the conservative fallback.

The transcript records only endpoints that were really invoked, with their status and duration; router previews and skipped variants are not reported as “models used.”
...
Gemini's OpenAI-compatibility layer carries tool-call **thought signatures** at `tool_calls[].extra_content.google.thought_signature` (confirmed against the Vercel AI SDK source), not at a top-level `thought_signature`. The adapter read only the latter, so Gemini 3 rejected every follow-up tool round with "Function call is missing a thought_signature". `openai-compatible.ts` now reads either location and echoes the signature back in the shape the provider expects — nested under `extra_content.google` for provider `google`, top-level for others.
...
`CopilotMultiplierSync` reads these values, and Copilot's token prices, from GitHub's documentation tables. The token-price parser locates the Input and Output columns **by header, per table**, because GitHub added release-status, category, tier and threshold columns and a fixed column index silently read the wrong numbers; it keeps the `Default` tier over long-context rows and drops `<sup>` footnote markers from model names. A failed fetch is retried at most hourly. Provider-model refreshes are coalesced — one in flight plus one queued — and VS Code's `onDidChangeChatModels` is debounced by 1.5 s, since it fires in bursts.
