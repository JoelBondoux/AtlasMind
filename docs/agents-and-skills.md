...
Continuation prompts are now mode-aware. A short follow-up such as `proceed with the fix` stays in normal freeform execution unless the operator explicitly asks for autonomous or project execution, or the recent transcript already shows an active project run. Atlas also suppresses the extra `Do you want me to fix this?` footer when the original prompt already describes a concrete workspace change such as moving, replacing, or renaming UI elements.
...
- Provider or routing failures now leave the affected subtask in a failed state instead of showing it as completed with only an error string, which keeps Project Run Center telemetry aligned with what actually executed.
