...
| `atlasmind.memory.backgroundSummarizationMode` | `"off"` \| `"local-only"` \| `"routed"` | `"off"` | Whether a background timer may refresh project-memory snippets using a model. **Off by default**: this path reads project-memory file content and sends up to 4,000 characters of it to the selected provider. `local-only` is enforced against the *resolved* provider and never falls back to a cloud or subscription provider; `routed` names each external destination in the output channel. An unrecognised value resolves to `off`. |
| `atlasmind.memory.selfHealingMode` | `"off"` \| `"report-only"` \| `"ask"` \| `"apply"` | `"report-only"` | What background memory maintenance may do to project files. Under the default it may detect and report but **never write**. Anything computed and withheld is reported, so "nothing happened" and "something was withheld" stay distinguishable. An unrecognised value resolves to `report-only`. |
| `atlasmind.cost.comparisonModel` | `string` | `""` | Model id to re-price spend against, e.g. `anthropic/claude-opus-5`. Empty means no comparison is made — AtlasMind will not pick one, because the choice decides what a saving is a saving *against*. Requests that cannot be re-priced honestly are excluded from both sides and reported separately, never counted as a zero saving. |
...
The Dashboard's Atlas icon is a submit action, not a copy-to-composer shortcut. Chat & Sidebar settings shows that same prompt button beside the selector so the control is visually identifiable; clicking the example button sends a clearly labelled real prompt through the selected declared route, which can consume that service's quota or incur its normal cost. `atlasmind` starts a new AtlasMind session and submits immediately. `vscode` submits through whichever target, agent, and model VS Code Chat currently has selected. Settings also lists installed extensions whose manifests declare a chat participant (`participant:<id>`) or chat session (`session:<type>`); an Open/Focus command alone is not treated as proof that an extension can receive a prompt.

Some extensions contribute several internal participant IDs for editor, notebook, agent, terminal, or other host contexts. Those are implementation routes, not separate installed chat services, so Settings exposes one primary participant per extension. The hidden IDs remain aliases for existing workspace selections. Declared chat sessions such as a CLI or cloud-agent session remain separate because VS Code exposes them as distinct destinations.

The destination is stored per workspace and re-resolved against the installed extensions on every click. An Extension Development Host opened before this setting was contributed can show the current Settings webview while VS Code still rejects the setting write; AtlasMind detects that stale registry, retains the validated selection in workspace-local extension state, uses it immediately, and asks for a window reload. An explicit registered configuration value takes precedence, and a successful settings write clears the fallback. A malformed, removed, or unsupported destination fails closed and sends nothing.

For an external destination, AtlasMind creates the Dashboard prompt and hands that text to the selected chat. From that point the external service controls the model, privacy and retention behaviour, quota or cost, and permission prompts. AtlasMind sends no private structured Dashboard context, but it also cannot apply its own redaction, spending limits, or approval policy after the hand-off; the generated prompt itself can contain project details.
...
A turn that outlives its window is still spending money and may still be editing files. A **status-bar item** names what is running and the `AtlasMind: Show Chats Running in the Background` command reads or stops any of them — closing the window is no longer the way to stop a run, so that is the way that replaces it. **Reopening the chat adopts the run**: the answer streams in, the stop button works, and asking something else starts a new conversation rather than interleaving two answers into one transcript. A prompt *queued* behind the running one is dropped rather than started, since finishing work already underway is a smaller step than beginning new work with no window. Turn the setting off to get the old behaviour exactly.
