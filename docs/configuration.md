...
| `atlasmind.acp.hideConsoleWindows` | `boolean` | `false` | **Windows only.** Keep the ACP agent and its descendants on a dedicated non-interactive window station and private desktop so consoles cannot appear on the input desktop or steal focus. Windows permits visible UI only on `WinSta0`; a descendant that chooses a new desktop therefore remains non-interactive. Setup asks before its first probe and records that guided choice in User settings (so setup does not dirty the repository); an explicit workspace value may still override it. Ordinary launching remains the compatibility-first choice. The bundled helper is SHA-256-pinned, receives an already-resolved executable plus argv (never a shell command), applies the creator token's default ACL to the station/desktop, inherits only stdin/stdout/stderr, never switches stations, and fails visibly if absent, changed, or blocked. The child still has the same user's filesystem/network authority: this is not a sandbox. Unusual hidden UI boundaries and a non-Authenticode-signed helper may be blocked by application control or EDR; the hash pin establishes expected bytes, not Windows reputation. Safe live-session reuse reduces visible starts even while this is off. Editable through the guided picker, the **Delegated agents (ACP)** card on Settings → Safety & Verification, and VS Code's settings editor. |
...
| `atlasmind.acp.toolsEnabled` | `boolean` | `false` | **Let subscription agents act** — the global ceiling that makes ACP agents eligible for tool-backed routing. The Orchestrator must separately authorize the exact provider request; ordinary ACP completions remain settings-isolated, receive no configured MCP servers, and have no approval policy even while this is on. On an authorized tool-backed turn AtlasMind sends no function schemas, stands down its own tool loop, and the work runs inside the agent's process with each operation approved. An empty `acp.mcpServers` allowlist does not disable built-in tools on that authorized turn. AtlasMind never selects `allow_always`, and a missing or throwing approval gate is a refusal. |
