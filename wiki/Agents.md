...
After AtlasMind selects an agent, it resolves the execution model separately. A skill-bearing task normally requires AtlasMind `function_calling`. When **Let subscription agents act** (`atlasmind.acp.toolsEnabled`) is enabled, an ACP subscription model declaring delegated native-tool execution can satisfy that requirement: AtlasMind passes no skill schemas and authorizes only that exact provider request, the agent uses its own tools, and each operation still returns through the ACP approval broker. The global setting alone does not authorize an ordinary completion. Turning it off leaves ACP available for tool-free chat/reasoning but excludes it from tool-backed routes; pins, allowlists, health, privacy, and normal routing still apply.
