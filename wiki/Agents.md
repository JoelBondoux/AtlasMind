...
- **Scaffold framework** (`AtlasMind: Scaffold Testing Framework`) infers the project stack and generates fitting starter files for each enabled methodology — Vitest/Jest specs, Playwright/Cypress e2e, fast-check property tests, k6 load scripts, snapshot tests — plus a managed `project_memory/operations/testing-strategy.md` playbook. Non-destructive: files are created only when absent, `package.json` is never mutated, and the run is modal-confirmed.
