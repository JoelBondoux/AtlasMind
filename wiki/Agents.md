...
After AtlasMind selects an agent, it resolves the execution model separately. A skill-bearing task normally requires AtlasMind `function_calling`. When **Let subscription agents act** (`atlasmind.acp.toolsEnabled`) is enabled, an ACP subscription model declaring delegated native-tool execution can satisfy that requirement: AtlasMind passes no skill schemas and authorizes only that exact provider request, the agent uses its own tools, and each readable operation request is automatically acknowledged once and logged. The global setting alone does not authorize an ordinary completion. Turning it off leaves ACP available for tool-free chat/reasoning but excludes it from tool-backed routes; pins, allowlists, health, privacy, and normal routing still apply.
...
ACP native tools are not an extra AtlasMind skill set. Provider discovery declares only that the agent supports delegated execution; the live tools-enabled setting makes that shape eligible, and the Orchestrator stamps only the selected tool-backed provider request with authority. The adapter requires all three facts before it drops completion isolation or wires its permission policy. It then stands down AtlasMind's own function loop for that attempt, while ordinary-provider failover restores the original tools.
