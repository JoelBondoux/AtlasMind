...
| `AtlasMind: Lens: Review Contract Wiring` | Scans a bounded set of supported TypeScript, OpenAPI, JSON Schema, and SQL declarations, asks for an ordered same-root pair, applies `.atlasmind/lens-mappings.json`, and opens the Field Wiring board. It imports/executes no project module, runs no SQL, database connection, or model |
