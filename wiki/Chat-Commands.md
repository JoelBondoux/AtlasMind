...
| `AtlasMind: Lens: Review Contract Wiring` | Scans a bounded set of supported OpenAPI, JSON Schema, and SQL declarations, asks for an ordered same-root pair, applies `.atlasmind/lens-mappings.json`, and opens the Field Wiring board. It runs no SQL, project code, database connection, or model |
