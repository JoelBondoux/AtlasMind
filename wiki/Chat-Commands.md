...
| `AtlasMind: Lens: Open Atlas Lenses Dashboard` | Opens one page for all eight lenses: what each one reads, the question it answers, whether it can answer it right now, and the declared rule behind that verdict. A flow map draws evidence → lens → question with hover and keyboard highlighting; a ⓘ on every card explains the lens in plain language and, separately, what it cannot prove. A **Do this next** band ranks only what needs a person and is empty when nothing does. Opening it runs no model, writes no file, and scans no workspace — evidence it did not inspect is reported as unassessed rather than as absent |
