...
| `AtlasMind: Install Recommended MCP Server` | One-click install and connect a curated AtlasMind-ready MCP preset, and bootstrap the missing runtime through a supported package manager on Windows, macOS, or Linux when that preset depends on a local CLI |
