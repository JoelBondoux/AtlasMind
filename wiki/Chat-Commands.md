...
| `/acp` | Guided ACP setup, five steps: name an agent → install it → sign in → enable the provider → **prove a completion comes back**. Same shape as `/buzz` — literally the same mechanics (`setupWalkthrough.ts`), so the two guides cannot drift. State is derived from your actual configuration, never asked for; the last step exists because configured and working are different things the settings screen cannot tell apart. Suggests Claude, Codex, Gemini, Copilot and Qwen with the exact command to run — including the `--acp` flag where one is needed — and the install command for each, derived from the same list the adapter spawns from so the two cannot disagree. **Nothing is installed or enabled for you.** |
