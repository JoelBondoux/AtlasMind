...
| `/acp` | Guided ACP setup, six steps: name an agent → **choose Windows console behaviour before the first probe** → install it → sign in → enable the provider → **prove a completion comes back**. Visible launching explains the possible terminal pop-ups; the private-desktop choice explains the hVNC/EDR trade-off and remains a Settings checkbox. Same mechanics as `/buzz`, with state derived from real configuration. Suggests Claude, Codex, Gemini, Copilot and Qwen with the exact command and install package derived from the same list the adapter spawns. **Nothing is installed or enabled for you, and dismissing the console picker stores nothing.** |
...
| `AtlasMind: Lens: Review Contract Wiring` | Scans a bounded set of supported TypeScript, OpenAPI, JSON Schema, and SQL declarations, asks for an ordered same-root pair, applies `.atlasmind/lens-mappings.json`, and opens Field Wiring plus any declared SQL relationship cards touching the pair. It imports/executes no project module, runs no SQL, database connection, or model |
...
| `More Target Actions…` | Lens file or symbol context menu | Offers Explain, Show impact, and Find tests as focused editable drafts. Symbols also offer **Trace possible flow**, an editor graph of bounded call-hierarchy/reference evidence with exact source and Ask Atlas actions. Drafts never auto-submit |
