...
| **AcpAdapter** | `src/providers/acp.ts`, `src/providers/acpProtocol.ts` | Agent Client Protocol provider: drives Claude Agent / Codex / any ACP agent as subscription-backed completion capacity. Contract read from the published spec and pinned (v1, JSON-RPC 2.0 over stdio). Streams `session/update` chunks, has no argv prompt ceiling, supports images when declared. Restricted mode (no fs, no terminal, no MCP pass-through) and fails closed on permission requests — delegated execution is Tier 3. Verified launch commands only; user-authored, seeded disabled. Wire framing is pure and unit-tested; the child process is injected so the state machine is tested without spawning |
