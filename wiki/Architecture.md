...
| **AcpSetupPlan** | `src/core/acpSetupPlan.ts` | The `/acp` walkthrough — name an agent, install it, sign in, enable the provider, prove a completion comes back. Derived from configuration; only published launch commands named; install commands quoted rather than offered as buttons |
...
| **ACP subscription offers** | `src/providers/acp.ts` (`ACP_PROVIDER_BRIDGES`) | Puts the ACP offer on the card of the pay-per-token provider it replaces — "Use my Claude subscription" on Anthropic, "Use my ChatGPT subscription" on OpenAI — because a protocol name is not something users look for. Clicking it is the whole discovery path: not installed offers the setup guide, signed-out says so, ready registers the agent and enables the provider. Only vendors with a published launch command appear, asserted against `VERIFIED_ACP_AGENTS` |
| **AcpAdapter** | `src/providers/acp.ts`, `src/providers/acpProtocol.ts` | Agent Client Protocol provider: drives Claude Agent / Codex / any ACP agent as subscription-backed completion capacity. Contract read from the published spec and pinned (v1, JSON-RPC 2.0 over stdio). Streams `session/update` chunks, has no argv prompt ceiling, supports images when declared. A completion source by default (no MCP pass-through, permission requests refused); `atlasmind.acp.toolsEnabled` enables delegated execution through `acpPermission.ts`. `fs`/`terminal` client capabilities stay `false` even then — they decide *who performs* an operation, not whether it may happen, and a coding agent already has its own access. Verified launch commands only; user-authored, seeded disabled. Wire framing is pure and unit-tested; the child process is injected so the state machine is tested without spawning |
