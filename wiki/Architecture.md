...
The Project Ideation panel is AtlasMind's dedicated multimodal whiteboard. It keeps draggable cards, link lines, focused-card inspection, queued Atlas follow-up prompts, facilitation history, browser-side voice capture and narration, drag/drop and paste-driven media ingestion, and inline double-click editing in one surface. Ideation board state persists into `project_memory/ideas/` as both JSON and markdown artifacts so the same project-memory system can retain ideation output.
