...
| **BuzzSetupPlan** | `src/core/buzzSetupPlan.ts` | The ordered Buzz setup checklist behind `/buzz`: master switch → relay → agent key → subscription, then the optional extras (persistence, CLI, MCP bridge). **A plan, never an installer** — every action opens a surface; nothing enables a gate, writes a setting, stores a secret, or connects anything, since flipping the deny-by-default switches to be helpful would remove the property they exist to provide. **Derived, not model-generated.** Refusals carry their reason (a plaintext remote relay says *why*). `nextBuzzSetupStep` is scoped to the required steps so a step blocked only by an optional prerequisite is never nominated. Pure + unit-tested |
