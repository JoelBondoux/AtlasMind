...
In the Agent Manager webview, the Agent Directory now includes a global Agent Auto-Update cadence selector. That control updates `atlasmind.agentAutoUpdateCadence` through validated extension-host message handling, so cadence changes are available directly alongside per-agent auto-update exclusions.
