...
Seven panels (Voice, Vision, Specialist Integrations, Tool Webhooks, Model Providers, Agent Manager, MCP) share one vertical tab controller, `PANEL_NAV_JS` in `src/views/panelNav.ts`. Each had carried its own copy of the same nav, and every copy declared `role="tablist"` while shipping plain buttons — no `role="tab"`, `aria-selected`, `aria-controls`, `role="tabpanel"`, roving `tabindex` or keyboard handling. The controller upgrades the existing markup at runtime instead of replacing it, so panel styling is untouched, and panel-specific work (webview state persistence) rides along through an `onActivate` callback. The Settings panel keeps its own implementation, which was already correct and additionally degrades to plain in-page anchors when the script does not boot.
...
In the Agent Manager webview, the Agent Directory includes a global Agent Auto-Update cadence selector. That control updates `atlasmind.agentAutoUpdateCadence` through validated extension-host message handling, so cadence changes are available directly alongside per-agent auto-update exclusions. Built-in agents render that exclusion checked and disabled, mirroring the host-side `AgentAutoUpdater.isDue()` guard that rejects built-ins before any provider call.
