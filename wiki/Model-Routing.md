...
| **ACP Agents (subscription/license)** | `acp` | Subscription/license | User-authored agent list (`atlasmind.acp.agents`); models are `acp/<id>` | Drives any Agent Client Protocol agent (`claude-agent-acp`, `codex-acp`, `gemini --acp`, `copilot --acp`, `qwen --acp`, …) over JSON-RPC on stdio using that vendor's subscription or eligible product license. Streams, has no argv prompt ceiling, and sends images when the agent declares support. A completion source by default. With `atlasmind.acp.toolsEnabled`, the router may satisfy a tool-backed task through the model’s distinct delegated-tool capability; the Orchestrator authorizes only that provider request, sends no function schemas, and readable native operation requests are automatically answered `allow_once` and logged. Ordinary completions remain isolated even while the setting is on. Declares `vision` once a handshake reports image support; never claims `function_calling`. Seeded disabled — nothing is spawned until you name a command you have installed. See [ACP agents](#acp-agents) below |
...
- **Where Windows may show them:** setup asks before the first probe. Ordinary launching is the compatibility-first default. The opt-in `atlasmind.acp.hideConsoleWindows` checkbox sends the already-resolved agent through a bundled, SHA-256-pinned Rust supervisor. It creates a token-ACL-scoped non-interactive window station and its `Default` desktop with the documented non-interactive access sets, then creates the agent suspended with `STARTUPINFO.lpDesktop = NULL` so it inherits that established connection. `CREATE_NEW_CONSOLE` plus `STARTF_USESHOWWINDOW`/`SW_HIDE` gives the complete ACP tree one non-visible console to inherit; the old `CREATE_NO_WINDOW` root had no console to pass down, so a later shell could allocate its own visible `conhost.exe`. The restricted stdin/stdout/stderr handle list preserves JSON-RPC pipes. Inherited error-mode flags turn a descendant loader failure into a process failure instead of an unattended modal dialog. The helper assigns the root to a kill-on-close Job Object before resuming it. Windows permits visible UI only on `WinSta0`, so even a descendant that chooses a new desktop cannot display UI or take focus. Npm adapters are started by a real `node.exe`, never VS Code's GUI `Code.exe`.
...
Windows 11 24H2 / Server 2025 also exposes [`AllocConsoleWithOptions(ALLOC_CONSOLE_MODE_NO_WINDOW)`](https://learn.microsoft.com/en-us/windows/console/allocconsolewithoptions), which could make the inherited console truly windowless on those systems. It is not the compatibility baseline: the API is build-26100-only, explicitly detached/new-console children can escape it, GUI windows still need the station boundary, and console attachment can alter stdio/TTY behaviour. Upstream fixes in each ACP agent remain the cleanest ownership/signing answer. ConPTY changes a raw JSONL protocol into terminal I/O; a Windows service/WSL/container/sidecar adds installation, identity, filesystem and credential plumbing; an in-process rewrite loses generic ACP isolation. The Job Object improves whole-tree teardown but does not itself hide windows.
...
Switching on **Let subscription agents act** makes ACP eligible for tool-backed work and is standing authorization for operations within those routed turns; it does not authorize every ACP prompt. The Orchestrator stamps only a selected tool-backed provider request with per-turn delegated authority. The adapter requires that stamp and the live setting before dropping isolation, sharing the allowlisted MCP servers, or wiring the permission policy. An omitted/false stamp remains completion-only even while the checkbox is on. Within an authorized turn, readable operation requests are automatically answered once and recorded without repeated dialogs. An empty MCP allowlist does not erase built-in agent tools. A change in either setting or request authority replaces an incompatible live session.
