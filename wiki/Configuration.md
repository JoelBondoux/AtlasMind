...
| `atlasmind.acp.toolsEnabled` | boolean | `false` | **Let subscription agents act** — the global ceiling that makes configured ACP agents eligible for tool-backed routing. The exact provider request must also carry Orchestrator authority; ordinary completions remain settings-isolated, receive no configured MCP servers, and have no approval policy even while this is on. An authorized tool-backed turn sends no AtlasMind schemas and uses the agent’s own tools, one approval at a time. An empty MCP allowlist does not disable built-in tools on that authorized turn. |
