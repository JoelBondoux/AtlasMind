...
| `atlasmind.workflow.chatGuidance` | string | `inform` | What AtlasMind does when a chat request would commit, push, branch, open a PR or release. `inform` states what the workflow expects and offers both paths; `gate` refuses until you say go ahead; `off` is silent. The chat path never read the workflow before this — it lived on a dashboard page and in *other* tools' instruction files. Detection is a published keyword table, not a model, so the same prompt always gives the same notice. Silent when no workflow is declared or the owning stage is disabled. |
...
| `atlasmind.acp.toolsEnabled` | boolean | `false` | **Let subscription agents act** — make configured ACP agents eligible for tool-backed routing and let the selected agent use its own tools, approving each operation. Off by default: ACP remains completion-only for routing. On: AtlasMind sends no function schemas and stands down its own tool loop; an empty MCP allowlist does not disable the agent’s built-in tools. AtlasMind never accepts “always allow,” and a missing approval gate denies rather than opens. |
...
| `atlasmind.acp.hideConsoleWindows` | boolean | `false` | Windows only. Put ACP agents and descendants on a dedicated private desktop so console windows cannot appear or steal focus. Setup asks before the first ACP probe and records the guided choice in User settings rather than dirtying the repository; workspaces can still override it explicitly. Off/ordinary is the compatibility-first default; the opt-in helper is SHA-256-pinned, uses no shell, passes only stdio, and fails visibly rather than falling back. Hidden desktops are also an hVNC malware technique, so enterprise EDR may flag or block this legitimate use. The v0.230.0 PE is not Authenticode-signed; the hash pin is an integrity check, not a Windows reputation signal. The same value is reachable from the guided picker (**AtlasMind: Choose ACP Console Window Behaviour**), the **Delegated agents (ACP)** card on Settings → Safety & Verification, and VS Code's settings editor. |
