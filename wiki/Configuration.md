...
| `atlasmind.acp.toolsEnabled` | boolean | `false` | **Let subscription agents act** — allow ACP agents to run their own tools, approving each operation. This is also the native VS Code Settings search wording. Off by default. AtlasMind never accepts an agent's "always allow" — it answers "allow once", so no grant ends up somewhere you cannot revoke it — and a missing approval gate denies rather than opens. |
