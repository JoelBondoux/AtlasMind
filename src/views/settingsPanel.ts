...
import {
  ...
  type TechnicalControlResult,
  type TechnicalControlSummary,
} from '../core/complianceTechnicalControls.js';
import { LENS_ENDPOINT_FILE, normalizeLensEndpointFile } from '../core/lensEndpoints.js';
import { readDeliveryConfig } from '../core/deliveryManager.js';
import { readWorkflowConfig } from '../core/workflowConfig.js';
import { readWorkflowHistory, WORKFLOW_HISTORY_SSOT_PATH } from '../core/workflowAuditRecord.js';
import { hasProviderDataGovernance } from '../core/providerDataGovernance.js';
import { readRiskOversightConfig } from '../core/riskOversightManager.js';
...
export interface TestingDashboardSnapshot {
  ...
  /**
   * Per-governance-regime control checks run against the stack.
   *
   * Keyed by policy id, and only for the regimes that declare a technical
   * check. A regime absent from this map has none — which is the honest answer
   * to "can this protocol be automated at all?" and the thing to look at before
   * deciding whether it is worth keeping switched on.
   */
  technicalControls?: Record<string, {
...
export function collectTestingDashboardSnapshot(
  ...
  const complianceSignals = gatherComplianceStackSignals(workspaceRoot, {
    ...
    // Every enabled provider is a sub-processor: project text reaches it.
    // Omitted entirely when the router is not available rather than reported
    // as zero — "no provider is configured" and "we could not look" are
    // different facts, and only the second should read as unassessed.
    ...(atlasContext?.modelRouter
      ? {
        enabledProviderIds: atlasContext.modelRouter.listProviders()
 ...
 */
function gatherComplianceStackSignals(
  workspaceRoot: string,
  input: {
