...
export function buildReviewedPrLocalCiWorkflow(config: ReviewedPrLocalCiConfig): string {
  ...
  const setupPython = TRUSTED_LOCAL_CI_ACTIONS_REVIEWED.setupPython;
  return `name: AtlasMind reviewed PR local CI\n\n`
    ...
    + `      - name: Set up the supported Python floor\n`
    + `        # ${setupPython.release}; immutable reviewed pin.\n`
    + `        uses: ${setupPython.name}@${setupPython.sha}\n`
    + `        with:\n`
    + `          python-version: '3.11'\n\n`
