...
export function readSecurityReviewConfig(workspaceRoot: string): SecurityReviewConfig | undefined {
  try {
    const raw = readFileSync(path.join(workspaceRoot, SECURITY_REVIEW_SSOT_PATH), 'utf8');
    const parsed = JSON.parse(raw) as unknown;
    return isSecurityReviewConfig(parsed) ? parsed : undefined;
  } catch {
    return undefined;
  }
}
...
export class SecurityReviewManager {
  ...
  constructor(private readonly workspaceRoot: string | undefined) {
    this.config = workspaceRoot ? readSecurityReviewConfig(workspaceRoot) : undefined;
  }
  ...
  reload(): SecurityReviewConfig | undefined {
    this.config = this.workspaceRoot ? readSecurityReviewConfig(this.workspaceRoot) : undefined;
    return this.config;
  }
