 ...
 */
/**
 * Which part of a regime a control belongs to.
 *
 * Grouping matters because the two halves are done by different people at
 * different times. A control set that is all `technological` — which ISO and
 * SOC 2 both were here — reads as though a regime were an engineering task,
 * and the organisational half then never gets written down at all. Themes make
 * the omission visible in the document itself.
 */
type ComplianceTheme = 'governance' | 'people' | 'physical' | 'technological';

interface ComplianceProfile {
  regime: string;
  /** What must be decided before the mapping means anything. */
  scoping: string;
  controls: Array<{ ref: string; requirement: string; theme?: ComplianceTheme }>;
}

const COMPLIANCE_PROFILES: Partial<Record<TestingMethodologyId, ComplianceProfile>> = {
  // Both of these regimes shipped as an engineering checklist: ISO listed nine
  // A.8 controls and one A.5, SOC 2 had no CC1–CC5 at all. That is not a
  // curated subset, it is half a regime — an auditor opens with the control
  // environment and the risk assessment, and a mapping that never mentions
  // them describes a project that has not started. The governance half is
  // first below for the same reason.
  'iso-27001': {
    ...
    controls: [
      // ── A.5 Organisational ──
      { ref: 'A.5.1', requirement: 'Information security policy defined, approved and communicated', theme: 'governance' },
      { ref: 'A.5.2', requirement: 'Information security roles and responsibilities assigned', theme: 'governance' },
      { ref: 'A.5.7', requirement: 'Threat intelligence collected and acted on', theme: 'governance' },
      { ref: 'A.5.9', requirement: 'Inventory of information and other associated assets maintained', theme: 'governance' },
      { ref: 'A.5.15', requirement: 'Access control policy defined and applied', theme: 'governance' },
      { ref: 'A.5.19', requirement: 'Information security in supplier and sub-processor relationships', theme: 'governance' },
      { ref: 'A.5.23', requirement: 'Information security for use of cloud services', theme: 'governance' },
      { ref: 'A.5.24', requirement: 'Incident management planned and prepared for', theme: 'governance' },
      { ref: 'A.5.26', requirement: 'Response to information security incidents defined', theme: 'governance' },
      { ref: 'A.5.30', requirement: 'ICT readiness for business continuity', theme: 'governance' },
      { ref: 'A.5.31', requirement: 'Legal, statutory, regulatory and contractual requirements identified', theme: 'governance' },
      { ref: 'A.5.34', requirement: 'Privacy and protection of personally identifiable information', theme: 'governance' },
      { ref: 'A.5.35', requirement: 'Independent review of information security', theme: 'governance' },
      // ── A.6 People ──
      { ref: 'A.6.3', requirement: 'Information security awareness, education and training', theme: 'people' },
      { ref: 'A.6.6', requirement: 'Confidentiality or non-disclosure agreements in place', theme: 'people' },
      // ── A.7 Physical ──
      { ref: 'A.7.9', requirement: 'Security of assets off-premises (laptops, portable media)', theme: 'physical' },
      // ── A.8 Technological ──
      { ref: 'A.8.2', requirement: 'Privileged access rights restricted and reviewed', theme: 'technological' },
      { ref: 'A.8.8', requirement: 'Technical vulnerabilities identified and remediated', theme: 'technological' },
      { ref: 'A.8.15', requirement: 'Logging of user activities, exceptions and security events', theme: 'technological' },
      { ref: 'A.8.16', requirement: 'Monitoring activities to detect anomalous behaviour', theme: 'technological' },
      { ref: 'A.8.24', requirement: 'Use of cryptography governed by policy', theme: 'technological' },
      { ref: 'A.8.25', requirement: 'Secure development lifecycle defined', theme: 'technological' },
      { ref: 'A.8.28', requirement: 'Secure coding principles applied', theme: 'technological' },
      { ref: 'A.8.29', requirement: 'Security testing performed in development and acceptance', theme: 'technological' },
      { ref: 'A.8.31', requirement: 'Development, test and production environments separated', theme: 'technological' },
    ],
  },
  soc2: {
    ...
    controls: [
      ...
      { ref: 'CC1.1', requirement: 'Commitment to integrity and ethical values demonstrated', theme: 'governance' },
      { ref: 'CC1.2', requirement: 'Independent oversight of internal control exercised', theme: 'governance' },
      { ref: 'CC1.3', requirement: 'Structures, reporting lines and authority established', theme: 'governance' },
      { ref: 'CC1.4', requirement: 'Commitment to attract, develop and retain competent people', theme: 'people' },
      { ref: 'CC1.5', requirement: 'Individuals held accountable for their control responsibilities', theme: 'governance' },
      // ── CC2 Communication and information ──
      { ref: 'CC2.1', requirement: 'Quality information obtained to support internal control', theme: 'governance' },
      { ref: 'CC2.2', requirement: 'Control responsibilities communicated internally', theme: 'governance' },
      { ref: 'CC2.3', requirement: 'Relevant matters communicated to external parties', theme: 'governance' },
      // ── CC3 Risk assessment ──
      { ref: 'CC3.1', requirement: 'Objectives specified clearly enough to assess risk against', theme: 'governance' },
      { ref: 'CC3.2', requirement: 'Risks to the objectives identified and analysed', theme: 'governance' },
      { ref: 'CC3.4', requirement: 'Significant changes assessed for their effect on control', theme: 'governance' },
      // ── CC4 Monitoring ──
      { ref: 'CC4.1', requirement: 'Ongoing evaluations confirm the controls are present and working', theme: 'governance' },
      { ref: 'CC4.2', requirement: 'Control deficiencies evaluated and communicated to those who can act', theme: 'governance' },
      // ── CC5 Control activities ──
      { ref: 'CC5.1', requirement: 'Control activities selected and developed to mitigate risk', theme: 'governance' },
      // ── CC6–CC8 Logical access, operations, change ──
      { ref: 'CC6.1', requirement: 'Logical access controls restrict access to protected assets', theme: 'technological' },
      { ref: 'CC6.2', requirement: 'User registration and de-registration are authorised', theme: 'technological' },
      { ref: 'CC6.3', requirement: 'Access is modified and removed on role change or exit', theme: 'technological' },
      { ref: 'CC6.6', requirement: 'External access points are protected', theme: 'technological' },
      { ref: 'CC6.7', requirement: 'Data in transit and at rest is protected', theme: 'technological' },
      { ref: 'CC7.2', requirement: 'Anomalies are monitored and detected', theme: 'technological' },
      { ref: 'CC7.3', requirement: 'Security incidents are evaluated and responded to', theme: 'technological' },
      { ref: 'CC8.1', requirement: 'Changes are authorised, tested and approved before deployment', theme: 'technological' },
      // ── CC9 / Availability ──
      { ref: 'CC9.2', requirement: 'Vendor and business-partner risk assessed and managed', theme: 'governance' },
      { ref: 'A1.2', requirement: 'Backup and recovery meet availability commitments (if in scope)', theme: 'technological' },
    ],
  },
  'nist-800-53': {
...
/**
 * The controls a regime declares, for callers that need the set rather than the
 * document — chiefly the test that pins `DECLARED_CONTROL_COUNT` in
 * `complianceTechnicalControls.ts` to this list. Those two live in different
 * files and have no compiler relationship, so a count that drifted below the
 * real set would silently under-report how much is still a person's job.
 */
export function complianceControlsFor(
  id: TestingMethodologyId,
): ReadonlyArray<{ ref: string; requirement: string; theme?: ComplianceTheme }> {
  return COMPLIANCE_PROFILES[id]?.controls ?? [];
}

const THEME_HEADINGS: Record<ComplianceTheme, string> = {
...
/** Declaration order, so a mapping opens with the half an assessor opens with. */
const THEME_ORDER: readonly ComplianceTheme[] = ['governance', 'people', 'physical', 'technological'];

/** The control-mapping document for a compliance policy, when it has one. */
function complianceRecipe(
  id: TestingMethodologyId,
  automatedRefs: ReadonlySet<string> = new Set(),
): ScaffoldFile[] {
  ...
  const row = (control: { ref: string; requirement: string }): string => {
    // A pointer to the live check, never a copy of its result. The mapping is
    // never rewritten, so a verdict recorded here would be a fact about the day
    // it was scaffolded quietly presented as a current one.
    const evidence = automatedRefs.has(control.ref)
      ? 'AtlasMind checks part of this — see Testing → Policy coverage'
      : '_none recorded_';
    return `| \`${control.ref}\` | ${control.requirement} | Not assessed | ${evidence} | _unassigned_ |`;
  };

  // Grouped, and governance first. An ungrouped list of twenty-five reads as
  // an engineering backlog, and the organisational half — which is the half an
  // assessor opens with — gets skimmed past.
  const grouped = THEME_ORDER.flatMap(theme => {
    const controls = profile.controls.filter(control => (control.theme ?? 'technological') === theme);
    return controls.length === 0 ? [] : [
      '',
      `### ${THEME_HEADINGS[theme]}`,
      '',
  ...
  const automatedCount = profile.controls.filter(control => automatedRefs.has(control.ref)).length;
  ...
  const content = [
    ...
    ...(automatedCount > 0
      ? [
        `AtlasMind automatically checks **part of ${automatedCount}** of these controls against your`,
...
function recipeFiles(id: TestingMethodologyId, stack: DetectedStack): ScaffoldFile[] {
  ...
  // Which refs AtlasMind checks is a property of the regime, not of the
  // workspace, so it is read with no signals — every result comes back
  // `unknown` and only the control references are used. Passing real signals
  // here would put a verdict into a file that is never rewritten.
  const compliance = complianceRecipe(
    id,
    new Set(evaluateTechnicalControls(id, {}).map(result => result.controlRef)),
  );
