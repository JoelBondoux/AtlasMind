## Run an Autonomous Project

Use `/project` to decompose a goal into subtasks executed in parallel:

```
@atlas /project Add input validation to the API endpoints
```

AtlasMind will:
1. **Preview** the plan with estimated file impact
2. **Gate** large operations behind an approval prompt
3. **Checkpoint** files before writes so you can roll back
4. **Report** per-subtask cost, duration, and changed files
