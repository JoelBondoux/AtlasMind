...
describe.each(UI_STUDIO_REFERENCE_PROJECTS)('UI Studio reference: $id', reference => {
  it('migrates every legacy wireframe fact to v8 and preserves it through save/reopen', async () => {
    ...
    expect(read.config).toMatchObject({ version: 8, surfaceKind: reference.surfaceKind });
