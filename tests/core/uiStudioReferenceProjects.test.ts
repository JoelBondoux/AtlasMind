...
function expectTargetIndependentGraph(graph: UiDesignGraph): void {
  expect(Object.keys(graph).sort()).toEqual(['revision', 'screens', 'tokens']);
...
describe.each(UI_STUDIO_REFERENCE_PROJECTS)('UI Studio reference: $id', reference => {
  it('migrates every legacy wireframe fact to v7 and preserves it through save/reopen', async () => {
    ...
    expect(read.config).toMatchObject({ version: 7, surfaceKind: reference.surfaceKind });
