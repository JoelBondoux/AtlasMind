...
- [ ] If you double click on a roadmap canvas node then you zoom into it. #mvp <!-- rm:if-you-double-click-on-a-roa -->
- [ ] The search functionality on the roadmap flowcharts should identify the roadmap nodes, but keep the other nodes greyted out so the user can still see dependancies. If they click on a node the same functionality as currently exists takes over. <!-- rm:the-search-functionality-on -->
...
- [ ] **Prompt-injection defense** — the security-first positioning is hollow once MCP + autonomy + untrusted content combine. #mvp <!-- rm:prompt-injection-defense-the -->
- [ ] **Reasoning-budget routing** — extends multi-axis routing (an existing strength) and rides the biggest model-capability trend. #mvp <!-- rm:reasoning-budget-routing-ext -->
...
- [ ] **Not pursuing:** becoming a generic BYO-CLI-agent multiplexer — that is SUPACODE's category and undercuts AtlasMind's integrated routing / memory / cost / privacy differentiators. #mvp <!-- rm:not-pursuing-becoming-a-gene -->
