...
- [ ] Retire the `claude-cli` provider once ACP is proven. ACP supersedes it on every axis that mattered — it streams (the CLI bridge cannot), has no ~26,000-character argv prompt ceiling, and can carry images — so keeping both means maintaining two Claude-subscription paths and asking users to choose between them. **Blocker cleared in v0.209.0:** ACP has now been driven end to end through the compiled adapter against real binaries on Windows — `claude-agent-acp` 0.63.0 streamed a reply with `inputTokens: 2, outputTokens: 5`, and `codex-acp` 1.1.7 with `28693`/`6`. macOS is still unexercised, which is the remaining evidence gap rather than a reason the design is unproven. Sequence: (1) prove ACP against a real binary, (2) mark `claude-cli` deprecated in the provider panel and settings, (3) route new installs to ACP while leaving existing configs working, (4) remove the adapter, its `MAX_CLAUDE_CLI_*` truncation constants, and its tier table in the next MAJOR.
- [ ] Adopt ACP (Agent Client Protocol) so AtlasMind can drive Claude Code, Codex, and Gemini CLI subscriptions as routable capacity — replacing the argv-bounded, tool-free `claude-cli` bridge. Phased plan in `roadmap/acp-integration.md`: **Tier 1 ACP-as-provider shipped in v0.170.0** (streaming, no 26k prompt cap, images, restricted mode, contract verified against the published spec); Tier 2 multi-subscription fleet — **shipped in v0.209.0**: Codex, and Gemini/Copilot/Qwen, whose launch commands the ACP registry now publishes and which are transcribed into `VERIFIED_ACP_AGENTS` at a pinned version rather than fetched at runtime, Tier 3 delegated execution behind AtlasMind's approval gate, Tier 4 AtlasMind exposed as an ACP agent. Safety-critical constraint: delegated execution is never delegated authorization, and the Orchestrator's tool loop must stand down rather than nest inside the agent's. Tier 1 verification is complete: real agent binaries have completed turns on Windows (see the `claude-cli` retirement item above for the figures). #mvp
- [ ] Make the sidebar's **Ready to ship?** verdicts reflect real evidence, not just declared configuration. `promotionReadiness.ts` (v0.213.0) can only report what the delivery config *declares* — blocked / gated / clear — because `ProjectStateTreeProvider.compute()` is synchronous by design: it reads in-memory registries and shells out to nothing, since it recomputes on ten different events. So nothing in that section has seen the working tree, the version delta or live CI, and the copy says so in every tooltip rather than implying otherwise. **The work:** an async gather for the three facts the plan already computes (`gatherPromotionFacts`, `resolveLiveCiStatus`), with a cache keyed on branch + head SHA so ten refresh events do not become ten `gh` calls, and a `stale`/`unknown` verdict for when the cache is cold or `gh` is absent. **Two constraints that must survive it:** `unknown` must never render as ready — a go-light that has not read the evidence is worse than no light, which is the whole reason the current version refuses to say "safe"; and the row must still *open the plan* rather than promote, because the plan, its per-gate attestations and the protected-target type-to-confirm are the gate, and a one-click tree row would route around all three. Raised from the v0.213.0 review, where the synchronous limit was deliberately shipped rather than papered over.
