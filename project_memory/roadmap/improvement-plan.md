...
- [ ] **Game Dev (Phase 1 foundation delivered; scoped consumers next):** Unity, Unreal, Godot, Web-based — specified in [project-composition.md](project-composition.md); phased plan in [`project_memory/roadmap/game-engine-integration.md`](../project_memory/roadmap/game-engine-integration.md). #mvp <!-- rm:game-dev-unity-unreal-godot -->
...
- [x] The Road to MVP panel on the dash lists all teh mvp roadmap items, but they're a little too closely huddled making them a little hard to read. Give them a little more breathing room. Maybe even try giving them a little more width and double their padding? #mvp <!-- rm:the-road-to-mvp-panel-on-the -->
- [x] The Overview button on the Project Dashboard navigation is not quite aligned horizontally properly as the other buttons are. Also some buttons at certain window widths overlap the edge of their wrapper box. #mvp <!-- rm:the-overview-button-on-the-p -->
- [x] When clicking on the Roadmap button on the Project Dashboard, there should be an immediate and direct way to visibly see a way to add an item to the roadmap #mvp <!-- rm:when-clicking-on-the-roadmap -->
